...
<html>
  ...
  <script type="text/javascript">
	...
	function showInvoicesDetail(invoice_id,IS_ALREADY){
		$("#pactAffix_" + invoice_id).toggle(function(){
	        $(this).load("../servlet/defaultDispatcher?__action=invoiceManageCommand.showInvoicesDetail&invoice_id=" + invoice_id + "&IS_ALREADY=" + IS_ALREADY);
	    });
	}
	...
	<br/>
				...
				<table cellspacing="1" class="grid_table">
					<tr>
						...
						<th style="text-align: center;" rowspan="2">
							<page:pagingSort pagingInfo="${pagingInfo }" orderBy="TOTAL_MONEY - INVOICE_MONEY1">剩余金额(A-B)</page:pagingSort>
						</th>
						<th style="text-align: center;" rowspan="2">
							操作
						</th>
					...
					<tr>
						<th style="text-align: center;">
							<page:pagingSort pagingInfo="${pagingInfo }" orderBy="INVOICE_COUNT1_1">已收件数量</page:pagingSort>
						</th>
						<th style="text-align: center;">
							<page:pagingSort pagingInfo="${pagingInfo }" orderBy="INVOICE_MONEY1_1">已收件金额(B)</page:pagingSort>
						</th>
						<th style="text-align: center;">
							<page:pagingSort pagingInfo="${pagingInfo }" orderBy="INVOICE_COUNT1_0">未收件数量</page:pagingSort>
						</th>
						<th style="text-align: center;">
							<page:pagingSort pagingInfo="${pagingInfo }" orderBy="INVOICE_MONEY1_0">未收件金额</page:pagingSort>
						</th>
						<th style="text-align: center;">
					...
					<tr>
						<td style="cursor: pointer;" onclick="showInvoicesDetail('${item.ID}',<c:if test="${item.IS_ALREADY == null}">0</c:if><c:if test="${item.IS_ALREADY != null}">${item.IS_ALREADY}</c:if>);">
						...
						<td>${item.INVOICE_COUNT1_1 }&nbsp;</td>
						<td><fmt:formatNumber type="currency" value="${item.INVOICE_MONEY1_1 }"/>&nbsp;</td>
						<td>${item.INVOICE_COUNT1_0 }&nbsp;</td>
						<td><fmt:formatNumber type="currency" value="${item.INVOICE_MONEY1_0 }"/>&nbsp;</td>
						<td>${item.INVOICE_COUNT }&nbsp;</td>	
					 	...
					 	<td><fmt:formatNumber type="currency" value="${item.REMANENT_MONEY}"/>&nbsp;</td> 
					   	<td>
					   		<c:if test="${item.IS_ALREADY eq 1}">
					   			<a href="#" onclick="alert('该合同发票已正常收件.如需处理,请先洽业管退件!!');" style="color: #bbbbbb;">添加发票</a>
					   		</c:if>
					   		<c:if test="${item.IS_ALREADY eq 0 || item.IS_ALREADY==null}">
