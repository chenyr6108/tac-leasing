...
<input type="hidden" name="__action" value="anomalousProjectCommand.getAnomalousByNotApproved"/>
<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
 		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;异常案件--3天未过案</span>
</div>
...
<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="#table1">
	...
	<tr class="ui-jqgrid-labels">
		...
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<page:pagingSort pagingInfo="${pagingInfo }" orderBy="upperUser"><strong>办事处主管 </strong></page:pagingSort>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<page:pagingSort pagingInfo="${pagingInfo }" orderBy="sensorUser"><strong>客户经理 </strong></page:pagingSort>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<page:pagingSort pagingInfo="${pagingInfo }" orderBy="minCommitDate"><strong>最早提交风控时间</strong></page:pagingSort>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<page:pagingSort pagingInfo="${pagingInfo }" orderBy="dayDiff"><strong>距今天数</strong></page:pagingSort>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<strong>融资额</strong>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<strong>有无退回记录 </strong>
		</th>
		<th class="ui-state-default ui-th-ltr zc_grid_head">
			<strong>案件状态 </strong>
		</th>
	...
	<tbody id="grid-data">
		...
		<tr	class="ui-widget-content jqgrow ui-row-ltr">
			...
			<td><fmt:formatDate value="${item.minCommitDate }" pattern="yyyy-MM-dd"/>&nbsp;</td>
			<td>${item.dayDiff }&nbsp;</td>
			<td><fmt:formatNumber type="currency" value="${item.leaseRze }"/>&nbsp;</td>
			<td><c:choose><c:when test="${item.arg1 > 0 }">有</c:when><c:otherwise>无</c:otherwise></c:choose></td>
			<td><c:choose><c:when test="${item.state eq 1 }">评审中</c:when><c:otherwise>业务处理中</c:otherwise></c:choose></td>
