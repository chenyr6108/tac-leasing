...
<script type="text/javascript" >
	...
	function subCheckPayLines(){
 		...
 		if (checkCheckPayBlank(form.DRAWER.value)) {
   	 		alert("出票人不能为空！");
            return false;
        }
 		if (checkCheckPayBlank(form.CHECKOUT_DATE.value)) {
   	 		alert("出票日期不能为空！");
            return false;
        }
 		if (checkCheckPayBlank(form.CHECK_NUM.value)) {
   	 		alert("出票号码不能为空！");
            return false;
        }		
	    ...
	    var END_PERIOD_NUM = document.getElementsByName("END_PERIOD_NUM");
	      //var CHECK_TYPE = document.getElementsByName("CHECK_TYPE");
	 	 //0702zhangbo
	    ...
	    for (var i=0;i<CUST_NAME.length;i++){
	   	 	if (checkCheckPayBlank(CUST_NAME[i].value)) {
	   	 		alert("客户名称不能为空！");
	            return false;
	        }
	    ...
	    if (form.ID.value==""){
	    	$.ajax({
				...
				data:{__action:'rentContract.checkRentCheckNum',
					CHECK_NUM:form.CHECK_NUM.value.trim()},
				dataType:'json',
...
<body>
	<center>
		<form name="checkPayForm" id="checkPayForm" method="POST"  action="../servlet/defaultDispatcher?__action=rentContract.createRentCheckDetail">
			<br>
					...
					<td>
						<div
							...
							<span class="ui-jqgrid-title"
								style="line-height: 30px; font-size: 15px;">&nbsp;&nbsp;录入支票明细</span>
						</div>
						<div
							...
							<div class="zc_grid_body jqgrowleft">
								<table cellspacing="0" cellpadding="0" border="0"
									...
									<tr>
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="DRAWER" value="${rentCheckDetail.DRAWER}" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;<input type="hidden" name="ID" value="${rentCheckDetail.ID}"></td>		
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECKOUT_DATE" class="DATE_SPI" style=" width: 80px; height: 18px;" readonly="readonly" value="<c:choose><c:when test="${rentCheckDetail.CHECK_OUT_DATE eq '1900-01-01'}"></c:when><c:otherwise>${rentCheckDetail.CHECK_OUT_DATE}</c:otherwise></c:choose>" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>&nbsp;</td>
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECK_NUM" value="${rentCheckDetail.CHECK_NUM}" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECK_MONEY" value="${rentCheckDetail.CHECK_MONEY}" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><select name="MEMO" id="MEMO" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
											<c:forEach items="${memoList}" var="item1">
														<option value="${item1.FLAG }" <c:if test="${item1.FLAG eq MEMO}">selected="selected"</c:if>>
															${item1.FLAG }
														</option>
											</c:forEach></select>&nbsp;</td>
										<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><select name="CHECK_TYPE" id="CHECK_TYPE" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
											<c:forEach items="${checkTypeList}" var="item1">
														<option value="${item1.CODE }" <c:if test="${item1.CODE eq CHECK_TYPE}">selected="selected"</c:if>>
															${item1.FLAG }
														</option>
											</c:forEach></select>&nbsp;</td>											
									</tr>
									<tr>
										<td colspan="6">			
											...
											class="ui-state-default ui-th-ltr zc_grid_head">操作&nbsp;&nbsp;&nbsp;&nbsp;
													<c:choose>
														<c:when test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">
														</c:when>
														<c:otherwise>
																<a href="javascript:void(0)" style="cursor: pointer;"
																	onclick="addCustRentCheckInfoRow();" style="font-size: 14px;"><img src="${ctx }/images/u100.gif">&nbsp;添加客户合同期数</a>
														</c:otherwise>
													</c:choose>
													</td>
												</tr>
												...
												<tr>
													<td class="ui-widget-content jqgrow ui-row-ltr"><input type="text" id="CUST_NAME${ status.count }"	name="CUST_NAME" size="30" onblur="getCustRentLeaseCode(START_PERIOD_NUM${ status.count },END_PERIOD_NUM${ status.count },LEASE_CODE${ status.count },this.value,this);" value="${item.CUST_NAME }" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;<input type="hidden" id="CHECK_ID${ status.count }"	name="CHECK_ID_R" value="${item.CHECK_ID }"></td>
													<td class="ui-widget-content jqgrow ui-row-ltr">
														<select id="LEASE_CODE${ status.count }" name="LEASE_CODE" onchange="loadLeaseTerm(START_PERIOD_NUM${ status.count },END_PERIOD_NUM${ status.count },this.value);" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
													...
													<td class="ui-widget-content jqgrow ui-row-ltr">
														<select id="START_PERIOD_NUM${ status.count }" name="START_PERIOD_NUM" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
													...
													<td class="ui-widget-content jqgrow ui-row-ltr">
														<select id="END_PERIOD_NUM${ status.count }" name="END_PERIOD_NUM" <c:if test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
													...
													<c:choose>
														...
														<c:otherwise>
															<a href="#" onclick="deleteRow.call(this);"><span id="del${status.count+3 }"><img src="${ctx }/images/u103.gif">删除</span></a>
														</c:otherwise>
													</c:choose>
									...
									<tr>
										<td colspan="6" align="center"
											class="ui-widget-content jqgrow ui-row-ltr" style="height:10">&nbsp;</td>
									</tr>	
									<tr>
										<td colspan="2" align="center">&nbsp;</td>
										<td colspan="2" align="center" style="height:25">
													<c:choose>
														<c:when test="${rentCheckDetail.HW_CHECK_STATE eq 1 or rentCheckDetail.FINANCE_CHECK_STATE eq 1}">
														</c:when>
														<c:otherwise>
															<a href="#" name="search" id="buttonAdd" onclick="subCheckPayLines();"  class="blue_button">保存数据</a>
														</c:otherwise>
													</c:choose>
										</td>
										<td colspan="2" align="center">&nbsp;</td>
