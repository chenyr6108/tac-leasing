		...
		<tbody id="divCustLinkman">
			<c:forEach items="${creditList}" var="item" varStatus="status">
				<tr id="CustLinkman_${item.ID }" class="ui-widget-content jqgrow ui-row-ltr">
					...
					<td >
						<c:choose>
							<c:when test="${not empty item.FINANCECONTRACT_DATE}">已支付</c:when>
							<c:when test="${not empty item.RECT_ID}">已签合同</c:when>
							<c:otherwise>
								<c:if test="${empty item.WIND_STATE and empty item.CREDIT_ID}">未提案</c:if>
								<c:if test="${empty item.WIND_STATE and not empty item.CREDIT_ID}">已提交</c:if>
								<c:if test="${ item.WIND_STATE eq 1 and not empty item.CREDIT_ID}">无条件通过</c:if>
								<c:if test="${ item.WIND_STATE eq 3 and not empty item.CREDIT_ID}">不通过附条件</c:if>
								<c:if test="${ item.WIND_STATE eq 4 and not empty item.CREDIT_ID}">不通过</c:if>
							</c:otherwise>
						</c:choose>&nbsp;</td>
					<td>${item.SUPL_TRUE }&nbsp;</td>
				</tr>
			</c:forEach>
