...
<html>
	<head>
		<title>项目申请管理受理页面</title>	 
	<script type="text/javascript" src="${ctx }/project/js/project.js"></script>
	</head>
	<body>
		<form action="../servlet/defaultDispatcher" name="form1" id="form1" method="post">
			<input type="hidden" name="__action" id="__action" value="projectManage.projectMan">
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;项目申请管理受理页面</span>
		   	</div>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
				...
				<c:forEach items="${dw.rs}" var="item">
					<tr class="ui-widget-content jqgrow ui-row-ltr">
						...
						<td style="text-align: center;"><c:if test="${item.PRJT_STATUS eq 1}"><img src="${ctx }/images/u611.gif" title="已提交"></c:if><c:if test="${item.PRJT_STATUS eq 2}"><img src="${ctx }/images/u608.gif" title="受理"></c:if><c:if test="${item.PRJT_STATUS eq 3}"><img src="${ctx }/images/u617.gif" title="已驳回"></c:if></td>
						...
						<td style="text-align: center;"><a href="javaScript:void(0)" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=projectManage.selectProjectForShow&prjt_id=${item.PRJT_ID }'">查看</a>|<c:choose><c:when test="${item.PRJT_STATUS eq 1}"><a href="javaScript:void(0)" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=projectManage.selectProjectForShow&FLAG=2&prjt_id=${item.PRJT_ID }'">处理</a></c:when><c:otherwise><font color="#c0c0c0">处理</font></c:otherwise></c:choose></td>
					</tr>
				</c:forEach>
			</table>
