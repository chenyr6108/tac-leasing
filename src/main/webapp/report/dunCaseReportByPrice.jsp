...
function expand(iddiv,moneyType,date){
    $("#"+iddiv).toggle(function(){
    	var urlParams = "dun_date___"+ date + "@@moneyType___" + moneyType;
        $(this).load("../servlet/defaultDispatcher?__action=dunCaseCommand.queryDunCaseReportDetail&DATE="+date+"&moneyType="+moneyType+"&type="+$('input[name="type"]:checked').val()+"&byType="+$('#byType').val()+"&urlParams="+urlParams);
    });
}
function queryDetail(moneyType,startrange,endrange,date){
	window.location = "../servlet/defaultDispatcher?__action=dunTask.manage&dun_date="+date+"&moneyType="+moneyType+"&startrange="+startrange+"&endrange="+endrange;
}
</script>
</head>
<body>
	<form action="../servlet/defaultDispatcher" method="post">
		<input type="hidden" name="byType" id="byType" value="price" />
		<input type="hidden" name="__action"  value="dunCaseCommand.queryDunCaseReport" />
		<div style="font-size:24px; font-weight:bold; color:#2E6E9E; text-align:center;">逾期状况统计表5（按金额）</div>
		...
		<div id="type" style="margin-top:4px;">分类
			<input type="radio" value="dept" id="radio1" name="type" <c:if test="${type=='dept'||(type!='special'&&type!='orgUpUser'&&type!='orgUser') }">checked="checked"</c:if> /><label for="radio1">办事处</label>
					...
					<tr>
						<c:if test="${item.count1_7 != null }">
							<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center; white-space:nowrap;">
								<a href="javascript:void(0);" onclick="expand('pactAffix${index.count}','${item.remark  }','${DATE }')">${item.remark  }</a>
								&nbsp;
							</td>
						</c:if>
						<c:if test="${item.count1_7 == null }">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count8_14!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',8,14,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count15_30!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',15,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;">
							...
							<c:if test="${item.count31_60!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',31,60,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count61_90!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',61,90,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',8,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',31,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',31,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',91,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count91_180!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',91,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.countMoreThan181!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',181,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30+item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.remark}',8,'','${DATE }');">
	...
	<p align="right" style="font-weight:bolder;">编号：IT-RPT-26</p>
	<script>
	$(function () {
		var d=new Date();
		if($("#DATE").val() != ''){
