...
<html>
	<head>
		...
		function makeWriteBackSome(){
			if(confirm("确定对所选数据冲红？")){
				document.getElementById("decompose").submit();
			}
		}
		</script>
		<title>冲红单管理（分项）--冲红列表</title>
	</head>
	...
	<body>
		...
		<form name="decompose" id="decompose" action="../servlet/defaultDispatcher"
			method="post">
			<input type="hidden" name="__action" id="__action" value="checkDecompose.writeBackTranctionSome">
			...
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;冲红单管理（分项）--冲红列表</span>
		   	</div>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
				...
				<c:set value="0" var="num" ></c:set>
					<c:forEach items="${custDecomposeList}" var="custDecomposeList"
						...
						<c:if test="${custDecomposeList.FICB_ITEM != '待分解来款'}">
							<tr>
								<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
									${status.count }&nbsp;
								</td>
								<c:if test="${custDecomposeList.FICB_TYPE == 0}">
								<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
									<input type="checkbox" name="item_num" value="${custDecomposeList.FICB_ID}" >
								</td>
								</c:if>
								<c:if test="${custDecomposeList.FICB_TYPE == 2}">
								<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
									&nbsp;<!--  <input type="checkbox" name="item_num" value="${custDecomposeList.FICB_ID}" disabled="disabled">-->
								</td>
								</c:if>
								<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
					...
					<!-- <tr>
						<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
							<input type="hidden" name="item_count" id="item_count" value="${num +1 }" />
							${num +1 }
						</td>
						<td align="center" class="ui-widget-content jqgrow ui-row-ltr">
							&nbsp;
