...
$(function (){
	...
	$("#amount_input").bind("input",function(){
		...
		$("#handling_charge").val(handlingCharge);
		$("#amount_r").val(realAmount);
	});
	$.each(search_status, function(){
		$("#search_status_" + this).attr("checked", "checked");
	});
	var errorMsg_return = "${errorMsg_return}";
	if(errorMsg_return != ""){
		$("#reason_r").val("${fundReturn.reason}");
		$("#reason_r").change();
	}
});
...
function showClaimDetail(fiin_id){
	$("#pactAffix_" + fiin_id).toggle(function(){
        $(this).load("../servlet/defaultDispatcher?__action=decompose.showFundsDetail&fiin_id=" + fiin_id);
    });
}
...
function reasonOther(e){
	if ($(e).val() == -1) {
		...
		$("#amount_r").attr("readOnly", false);
		$("#amount_r").css("border", "1px solid #A6C9E2");
		$("#amount_input").val("");
		$("#handling_charge").val("");
		$("#amount_r").val("");
		...
		$("#amount_r").attr("readOnly", true);
		$("#amount_r").css("border", "1px solid #AFAFAF");
		$("#amount_input").val("");
		$("#handling_charge").val("");
		$("#amount_r").val("");
	} else {
...
<table class="grid_table">
	<tr>
		...
		<th style="width: 13%"><page:pagingSort pagingInfo="${dw }" orderBy="HAS_FILE">是否上<br>传水单</page:pagingSort></th>
		...
		<th style="width: 10%">(系统匹配)<br>对方户名</th>
		...
		<tr>
			...
			<td style="text-align:center">${fund.OPPOSING_UNIT_TRUE }&nbsp;</td>
			...
			<td style="text-align:right"><input name="input3" style="cursor:pointer" class="ui-state-default ui-corner-all" type="button" value="<fmt:formatNumber type="currency" value="${fund.INCOME_MONEY }"/>" class="button_keda" onClick="showCustRecp(${fund.INCOME_MONEY });">&nbsp;</td>
			...
			<td style="text-align: center;">
					<c:if  test ="${not empty fund.OPPOSING_MEMO}">
						<a href="#" onmouseover="mouseOver(event, '${fund.OPPOSING_MEMO}')" onmouseout="mouseOver(event, 0)">显示</a>
					</c:if> &nbsp; 
				</td>
			<td style="text-align:center">
				<c:if test="${fund.DECOMPOSE_STATUS != 7 }">
					<c:if test="${claimRole }">
						<a href="javascript:void(0);" onclick="showClaimReason(${fund.FIIN_ID},${fund.SURPLUS_MONEY })">[认领]</a>&nbsp;
					</c:if><c:if test="${returnRole }">
						<a href="javascript:void(0);" onclick="showFundsReturn(${fund.FIIN_ID},${fund.SURPLUS_MONEY })">[退款]</a>&nbsp;
					</c:if><c:if test="${authRole }">
						<a href="javascript:void(0);" onclick="passFundsReturn(${fund.FIIN_ID},${fund.SURPLUS_MONEY })">[确认]</a>
					</c:if>
				</c:if>&nbsp;
				<a href="javascript:void(0);" onclick="upLoadTransferCertificate(${fund.FIIN_ID},${fund.INCOME_FINANCE_CODE })">[上传水单]</a>&nbsp;
				<a href="javascript:void(0);" onclick="showMailMemo(${fund.FIIN_ID},'${fund.MAIL_MEMO}')">[修改备注]</a>
			</td>
		</tr>
...
<div style="display:none;" id="claimReason" title="请填写您的认领金额和原因">
			...
			<div class="ui-widget-content jqgrow ui-row-ltr ">
				<table cellspacing="1" cellpadding="3" border="0" class="ui-jqgrid-htable zc_grid_title">
					<tr class="ui-state-default ui-jqgrid-hdiv">
						<td style="text-align: center;"><strong>认领金额：</strong><font color="red">*</font></td>
						<td><input type="text" id="amount_win" name="amount_win" width="150" value="${amount }" class="tb_return"/></td>
					</tr>
					<tr class="ui-state-default ui-jqgrid-hdiv">
						<td style="text-align: center;"><strong>是否开立发票：</strong><font color="red">*</font></td>
						<td>
							<select name="is_openInvoice_win" id="is_openInvoice_win">
					...
					<tr class="ui-state-default ui-jqgrid-hdiv">
						<td style="text-align: center;"><strong>认领原因：</strong><font color="red">*</font></td>
						<td>
							<textarea id="reason_win" name="claimReason_win" rows="8" cols="55" style="border: 1px solid #A6C9E2;">${claimReason }</textarea>
						</td>
					</tr>
					<tr class="ui-state-default ui-jqgrid-hdiv">
						<td colspan="2" id="button_flag" style="text-align: center;">
							<input type="button" id="sumBut" value="确&nbsp;定" onclick="subFundsClaim()" class="ui-state-default ui-corner-all" >
							<input type="button" value="关&nbsp;闭" onclick="$('#claimReason').dialog('close');" class="ui-state-default ui-corner-all">
						</td>
					</tr>
				</table>
			</div>
			...
			<div class="ui-widget-content jqgrow ui-row-ltr ">
				...
				<table cellspacing="1" cellpadding="3" border="0" class="ui-jqgrid-htable zc_grid_title">
					<tr class="ui-state-default ui-jqgrid-hdiv">
						...
						<td><input type="text" name="payee" id="payee" class="tb_return" value="${fundReturn.payee }"/></td>
					...
					<tr class="ui-state-default ui-jqgrid-hdiv">
						...
						<td>
							<select name="reason" id="reason_r" onchange="reasonOther(this);" class="tb_return">
								...
								<%-- <option value="误汇款" <c:if test="${fundReturn.reason eq '误汇款' }">selected</c:if>>误汇款</option>
								<option value="供应商代垫租金" <c:if test="${fundReturn.reason eq '供应商代垫租金' }">selected</c:if>>供应商代垫租金</option>
								<option value="保险退款" <c:if test="${fundReturn.reason eq '保险退款' }">selected</c:if>>保险退款</option>
								<option value="保证金入我司" <c:if test="${fundReturn.reason eq '保证金入我司' }">selected</c:if>>保证金入我司</option>
								<option value="我司转入供应商" <c:if test="${fundReturn.reason eq '我司转入供应商' }">selected</c:if>>我司转入供应商</option> --%>
					...
					<tr class="ui-state-default ui-jqgrid-hdiv" id="handling_charge_item">
						...
						<td>
							<input type="text" id="amount_input" name="amount_input" value="${fundReturn.amount_input }" class="tb_return"/>
						</td>
					...
					<tr class="ui-state-default ui-jqgrid-hdiv">
						...
						<td><input type="text" name="amount" id="amount_r" value="${fundReturn.amount }" class="tb_return"/></td>
					...
					<tr class="ui-state-default ui-jqgrid-hdiv">
						...
						<td><input type="text" id="bank_name" name="bank_name" value="${fundReturn.bank_name }" class="tb_return"/></td>
						<td align="right">银行账号:</td>
						<td><input type="text" name="bank_account" value="${fundReturn.bank_account }" class="tb_return"/></td>
					</tr>
					<tr class="ui-state-default ui-jqgrid-hdiv">
...
<!-- Add by 沈祺 2013-7-4 -->
<!------------弹出层:增加不明款备注填写------------>
<div style="display: none;" id="memoDiv" title="修改备注">
	<div class="zc_grid ui-jqgrid ui-widget ui-widget-content ui-corner-all" id="main">
          <div class="zc_grid_body jqgrowleft">
			<div class="ui-widget-content jqgrow ui-row-ltr">
				<form name="memoForm" id="memoForm" method="post" action="../servlet/defaultDispatcher?__action=decompose.updateMailMemo">
				...
				<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
					...
					<tr>
						<td style="text-align: center;" colspan="2">
							<input type="button" class="ui-state-default ui-corner-all" value="保存" onclick="modifyMailMemo()">
						</td>
					</tr>
				</table>
				</form>
