	...
	<body>
		<form id="form1" name="form1" method="POST"  action="${ctx}/servlet/defaultDispatcher">
			...
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;法务拨款管理</span>
		   	</div>
							...
							<tbody>
								<c:forEach items="${dw.rs}" var="item" varStatus="status">													
									<tr class="ui-widget-content jqgrow ui-row-ltr">
										<td style="text-align: center;" onClick="exBankMoney('contractTr${item.RECT_ID }','${item.RECT_ID }','manage');"><c:if test="${item.PAY_MONEY eq null or item.PAY_MONEY eq 0}"><img src="${ctx }/images/u608.gif" title="未拨款"></c:if><c:if test="${item.PAY_MONEY > 0 and item.PAYCOUNT eq item.PAY_MONEY}"><img src="${ctx }/images/u611.gif" title="完成付款"></c:if><c:if test="${item.PAY_MONEY > 0 and item.PAY_MONEY < item.PAYCOUNT }"><img src="${ctx }/images/u622.gif" title="拨款中"></c:if></td>
										...
										<td style="text-align: right;"><fmt:formatNumber type="currency" value="${item.PAYCOUNT eq null ? 0 :item.PAYCOUNT }" />&nbsp;</td>
										<td style="text-align: right;"><fmt:formatNumber type="currency" value="${item.PAYCOUNT-item.PAY_MONEY }" />&nbsp;</td>
										<td style="text-align: center;">&nbsp;<fmt:formatDate value="${item.LESSOR_TIME }" pattern="yyyy-MM-dd"/></td>
										<td style="text-align: center;"><c:choose><c:when test="${item.PAY_MONEY > 0 and item.PAYCOUNT eq item.PAY_MONEY}"><font color="#c0c0c0">申请拨款</font></c:when><c:otherwise><a href="javaScript:void(0);" onclick="showEditPayMoneyReviewRecord('${item.LEASE_CODE }','${item.RECT_ID }');">申请拨款</a></c:otherwise></c:choose></td>
									</tr>
									<tr>
