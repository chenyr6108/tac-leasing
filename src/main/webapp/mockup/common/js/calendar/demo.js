...
$(document).ready(function() {
                ...
                buttons: {
                    save : function(){
                        ...
                        $.ajax({
												   ...
												   data: "__action=sys.newid",
												   cache: false,
													 async: false,
													 dataType: "json",
												   success: function(json){
												     alert(json.rs['uuid']);
														calEvent.id = json.rs['uuid'];
												   }
												});
                        ...
                        //alert(calEvent.start.format("yyyy-MM-dd hh:mm:00"));
                        
                        //save event
                        $.getJSON("/servlet/defaultDispatcher",
												{__action:"calendar.create", caldUid:calEvent.id, startTime:calEvent.start.format("yyyy-MM-dd hh:mm:00"), endTime:calEvent.end.format("yyyy-MM-dd hh:mm:00"), title:calEvent.title, content:calEvent.body },  
													function(json){
    ...
    function getEventData() {
      ...
      $.ajax({
			   type: "POST",
			   url: "/servlet/defaultDispatcher",
			   data: "__action=calendar.loadWeekCalendar",
			   cache: false,
				 async: false,
				 dataType: "json",
			   success: function(json){
