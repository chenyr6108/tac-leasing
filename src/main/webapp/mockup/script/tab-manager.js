$(function(){
	...
	function openTab(config){
		...
		var content = c.location?$("<iframe>").attr("frameBorder",0).attr("border",0).attr("name",c.tabId).attr("src",c.location+(c.location.indexOf("?")==-1?"?":"&")+new Date().getTime()):c.content;
		...
		var tab = $("<table>")
			...
			.append($("<tbody>").append($("<tr>")
				...
				.append($("<td>").attr("class","tdDis").append($("<nobr>").html(c.tabName)))
...
//		this.updateFrameMenu(c);
	...
	openTab({tabName:"我的桌面",location:"desktop.html",noClose:true});
