...
paylistchangeRZE = function () {
	...
	var totalFeeRZE=0;
	 $("input[feeListRZE='feeListRZE1']").each(function(){
		 ...
		 }else if(PAY_TAX_PLAN_CODE=="1"||PAY_TAX_PLAN_CODE=="3"||PAY_TAX_PLAN_CODE=="4"||PAY_TAX_PLAN_CODE=="5"||PAY_TAX_PLAN_CODE=="6"||PAY_TAX_PLAN_CODE=="7"||PAY_TAX_PLAN_CODE=="8"){
			 totalFeeRZE=totalFeeRZE+parseInt(Math.round(parseCurrency($(this).val())/1.06));
		 }
		 		 
	 });
	
	 if(PAY_TAX_PLAN_CODE=="5") {
		 ...
		 -parseInt(Math.round(parseCurrency($("#incomePayTax").val())))+parseInt(Math.round(parseCurrency($("#incomePay").val())))-parseInt(Math.round(parseCurrency($("#outPay").val())));
	 }
	 if(PAY_TAX_PLAN_CODE=="7") {
		 totalFeeRZE=totalFeeRZE-parseInt(Math.round(parseCurrency($("#pay__money").val())));
	 }
...
function submitCalCulate(flag,tax_plan_code) {
 //
	...
	*/
    ...
    if (flag == 1){
	    form.__action.value = "creditPaylistService.calculateCreditPaylist";
    
    } else if (flag == 2) {
	    form.__action.value = "creditPaylistService.calculateCreditPaylistIRR";
    
    } else if (flag == 3) {
	
	    form.__action.value = "expQuoToPdf.expZulwToPdf";
    
    }else if(flag == 4 && (tax_plan_code==1||tax_plan_code==3||tax_plan_code==5)){
    	form.__action.value = "creditPaylistService.exportPaylist"
    }else if(flag == 4 && tax_plan_code==2){
    	form.__action.value = "creditPaylistService.exportPaylistByValueAdded"
    }else if(flag==5){
    	//flag==5   支付表详细
    	form.__action.value = "creditPaylistService.paylistDetail"
    }
	form.submit();
}
function checkBZJ(){
...
function addSchema(word){ 
	...
	for (var i=0;i<PAY_IRR_MONTH_PRICE_END.length;i++){
		...
		if (i==PAY_IRR_MONTH_PRICE_END.length-1 && PAY_IRR_MONTH_PRICE_END[i].value != $("#PAY_LEASE_PERIOD").val()){
			alert("你选择的应付租金期数与租赁方案中租赁期数不符！");
			return false;
		}
    }
	...
	//
	PAY_PLEDGE_AVE_PRICE=parseCurrency($("#PAY_PLEDGE_AVE_PRICE").val()==""?"0":$("#PAY_PLEDGE_AVE_PRICE").val());//平均抵充金额
	...
	$.ajax({
		...
		data:'__action=creditPaylistService.saveSchemaToSession&PAY_LEASE_TOPRIC='+PAY_LEASE_TOPRIC
		...
		+'&TOTAL_VALUEADDED_TAX='+TOTAL_VALUEADDED_TAX+'&pay__money='+$("#pay__money").val()
		+'&incomePay='+$("#incomePay").val()
		+'&incomePayType='+$("#incomePayType").val()
		+'&outPay='+$("#outPay").val()
...
//					+"						</tr>"
					...
					if(word=='up'){
					    ...
					    +"							<td style='height: 23px;text-align: center'><a href='javascript:void(0)' onclick='javacript:location.href=\"../servlet/defaultDispatcher?__action=creditPaylistService.updateSchema&contract_type="+$("#contract_type").val()+"&session_schema="+i+"&CREDIT_ID="+$("#cccccc").val()+"\";'>保存</a></td>"
					    +"						</tr> ";
					}else{
						html=html+"						<tr class='ui-widget-content jqgrow ui-row-ltr'>"
...
function deleteDIV(obj,idValue,word) {
     ...
     $.ajax({
		...
		dataType:'json',
		url:'../servlet/defaultDispatcher',
		data:'__action=creditPaylistService.removeSchemaFromSession&idValue='+idValue,
		success:function(dataBack){
			  html="<td width='20%'> "
				    +"<div  style='width: 130px'> "
				    +"		<div class='ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix' style='height: 25px;text-align: right;' >"
