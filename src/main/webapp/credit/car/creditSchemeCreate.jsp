...
<html>
	<head>
		...
		<script type="text/javascript" src="${ctx }/credit/car/js/selectEquipment_new.js"></script>
		<script type="text/javascript" src="${ctx }/credit/car/js/selectInsureItem.js"></script>
		<script type="text/javascript" src="${ctx }/credit/car/js/creditScheme.js"></script>
		<script type="text/javascript" src="${ctx }/credit/car/js/creditSchemaIrrMonthPrice.js"></script>
		<script type="text/javascript" src="${ctx }/credit/car/js/selectOtherPrice.js"></script>
		...
		<script type="text/javascript" src="${ctx }/credit/car/js/autoquery.js"></script>
		...
		<script type="text/javascript">			
			function checkTime(){
				if($("input[name=functionFrom]").val()=='Y') {
					return true;//业务支撑不做验证
				}
        ...
        <div class="ui-state-default ui-jqgrid-hdiv " style="background-color: white; overflow: hidden !important;">
			<table cellspacing="0" cellpadding="5" border="0" class="ui-jqgrid-htable zc_grid_title" id="#table1">   				
				...
				<tr class="ui-widget-content jqgrow ui-row-ltr">
					...
					<td style="font-size: 14px;">
						<select id="contract_typew" name="contract_typew" onchange="clearUnitPrice();showIsNewProduction();showImportEqipItem(this.value);changePayWayForCar(this.value)"> 
							<c:forEach items="${contractType}" var="contractType">
								<c:if test="${contractType.CODE eq 6 or contractType.CODE eq 8 or contractType.CODE eq 10 or contractType.CODE eq 12 or contractType.CODE eq 13 or contractType.CODE eq 14}">
				...
				<tr class="ui-widget-content jqgrow ui-row-ltr">
					...
					<td  style="font-size: 14px;">
						<input type="radio" name="groupInside" value="1" id="groupInside" <c:if test="${creditMap.GROUP_INSIDE eq 1}">checked="checked"</c:if>><label for="groupInside">集团内</label>
						<input type="radio" name="groupInside" value="2" id="groupOutside"  <c:if test="${empty creditMap.GROUP_INSIDE or creditMap.GROUP_INSIDE eq 2}">checked="checked"</c:if>><label for="groupOutside">集团外</label>
					</td>
					<td colspan="5">&nbsp;</td>
				</tr>				
			</table>
...
<!-- Add by Michael 2012 01/14 费用参数化 -->					
				...
				<tr>
					...
					<td colspan="3" class="ui-widget-content jqgrow ui-row-ltr" style="font-size: 14px;">
						<input disabled="disabled" type="radio" id="OnlinetBank"  <c:if test="${schemeMap.APPROPRIATION_WAY eq 2 || creditMap.CONTRACT_TYPE eq 10 || creditMap.CONTRACT_TYPE eq 12 || creditMap.CONTRACT_TYPE eq 13}">checked="checked"</c:if>  value="2"><label for="OnlinetBank">网银</label>
						<input disabled="disabled" type="radio" id="cheque"  <c:if test="${schemeMap.APPROPRIATION_WAY eq 1 || creditMap.CONTRACT_TYPE eq 8 || creditMap.CONTRACT_TYPE eq 14}">checked="checked"</c:if> value="1"><label  for="cheque">支票</label>
						<c:if test="${schemeMap.APPROPRIATION_WAY eq 2 || creditMap.CONTRACT_TYPE eq 10 || creditMap.CONTRACT_TYPE eq 12 || creditMap.CONTRACT_TYPE eq 13}">
							<input type="hidden" name="appropriation_way" value="2">
						</c:if> 
						<c:if test="${schemeMap.APPROPRIATION_WAY eq 1 || creditMap.CONTRACT_TYPE eq 8 || creditMap.CONTRACT_TYPE eq 14}">
						<input type="hidden" name="appropriation_way" value="1">
						</c:if> 
						<span id="chequespan">
				...
				<tr id="chequeDiv" <c:if test="${schemeMap.APPROPRIATION_WAY ne 1 || creditMap.CONTRACT_TYPE eq 10 || creditMap.CONTRACT_TYPE eq 12 || creditMap.CONTRACT_TYPE eq 13}">style="display:none"</c:if>>
					<td class="ui-widget-content jqgrow ui-row-ltr" style="font-size: 14px;">
						背书人
					</td>
					<td colspan="3" class="ui-widget-content jqgrow ui-row-ltr" style="font-size: 14px;">
