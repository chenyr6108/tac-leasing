...
<html>
	<head>
		...
		<script src="${ctx }/credit/car/js/selectCard.js" type="text/javascript"></script>
		...
		function subRentFile_cust(){ 
			document.getElementById("buttonAddSub_cust").disabled=true;
			$("#rentForm").submit();
		}
		function subRentFile_vouch(){ 
			document.getElementById("buttonAddSub_vouch").disabled=true;
			$("#rentForm").submit();
		}
		...
		function expand(id,REFI_ID,PRCD_ID){	   
	        $("#"+id).toggle(function(){
	            $(this).load("../servlet/defaultDispatcher?__action=rentFile.queryFileUpMore&REFI_ID=" + REFI_ID+"&PRCD_ID="+PRCD_ID+"&DELETE_ENABLE=true");
	            $("#tr_"+id).toggle();
	        });
		}							
		</script>
			...
			<input type="hidden" id="rentFileFlag" name="rentFileFlag" value="${rentFileFlag}" />	
						...
						<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title grid_table">
								<tr class="ui-jqgrid-labels">
									...
									<th class="ui-state-default ui-th-ltr zc_grid_head" style="width:20%;">附件<br/>(注：弹出选择框中可选多张图片,单张图片大小不能超过2M)</th>
								</tr> 
								...
								<tr id="${filetype0}" class="ui-widget-content jqgrow ui-row-ltr" name="${filetype0}A">
									...
									<td width="8%" style="text-align: center;"><span>${item.FILECOUNT}</span>
										<input type="button"  class="ui-state-default ui-corner-all" value="查看" onclick="expand('pactAffix${item.REFI_ID}','${item.REFI_ID}','${prcd_id}');"  />
									</td>
									...
									<%-- name="fileselect[]"<td width="20%"><input class="ui-state-default ui-corner-all" size="13" type="file" name="${item.REFD_ID}@${item.REFI_ID}" id="${item.REFD_ID}@${item.REFI_ID}"/></td>
								 --%>
									<td width="20%">
									      <input  id="${item.REFD_ID}@${item.REFI_ID}" type="file" size="30"  name="${item.REFD_ID}@${item.REFI_ID}"  multiple  class="ui-state-default ui-corner-all" accept="image/gif,image/png,application/pdf,image/tiff" />
						    		</td>	
								</tr>
						...
						<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
							<tr>
							<td><input type="button" class="ui-state-default ui-corner-all" id="addLog1" value="提醒日志" name="addLog1" onclick="rentLog1();"><input class="ui-state-default ui-corner-all" type="button" value="保存数据" onclick="subRentFile_cust();" id="buttonAddSub_cust"/></td>
							<td></td>
							</tr>
						</table>							
		</div>
		...
		<div id="tabs-2">	
						...
						<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title grid_table">
								...
								<tr id="${filetype0}" class="ui-widget-content jqgrow ui-row-ltr" name="${filetype0}B">
									...
									<td width="8%" style="text-align: center;"><span>${item.FILECOUNT}</span>
										<input type="button"  class="ui-state-default ui-corner-all" value="查看" onclick="expand('pactAffix2${item.REFI_ID}','${item.REFI_ID}','${prcd_id}');"  />
									</td>
						...
						<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
							<tr>
							<td><input type="button" class="ui-state-default ui-corner-all" id="addLog2" value="提醒日志" name="addLog2" onclick="rentLog2();"><input class="ui-state-default ui-corner-all" type="button" value="保存数据" onclick="subRentFile_vouch();" name="buttonAddSub_vouch" id="buttonAddSub_vouch"/></td>
							<td></td>
							</tr>
						</table>						
		</div>
