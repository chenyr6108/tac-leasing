...
<html>
	<head>
		...
		function checkAll(e, itemName)
		{
	 	...
	 	{
	 		location.href='${ctx }/servlet/defaultDispatcher?__action=payMoney.payMoneyManagerByOneBonus&payMoneyId='+code;
	 	}
				...
				<div class="ui-state-default ui-jqgrid-hdiv ">
					<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="#table1">
						...
						<th style="text-align: center;height:26px;"  class="ui-state-default ui-th-ltr zc_grid_head">
							<page:pagingSort pagingInfo="${dw }" orderBy="APPLICATION_DATE">申请日期</page:pagingSort>
						</th>
						<th style="text-align: center;height:26px;" class="ui-state-default ui-th-ltr zc_grid_head">
							<page:pagingSort pagingInfo="${dw }" orderBy="PAY_DATE">实际支付日期</page:pagingSort>
						</th>
							...
							<tbody>
								<c:forEach items="${dw.resultList}" var="item" varStatus="status">													
									...
									<td style="text-align: center;"><c:if test="${item.STATE==1 }"><input type="checkbox" value="${item.ID }=${item.CREDIT_ID}=${item.BACKSTATE }=${item.LEASE_CODE }=${item.CUST_NAME }=bonus"  name="id|credit"></c:if>&nbsp;</td>
						...
						<td style='text-align: center;height: 25px;'><fmt:formatDate value="${item.PAY_DATE }" pattern="yyyy-MM-dd"/>&nbsp;</td>	
										...
										<td style="text-align: center"><c:if test="${item.BACKSTATE==0 }">设备款</c:if><c:if test="${item.BACKSTATE==1 }">保证金拨款</c:if><c:if test="${item.BACKSTATE==2 }">奖金拨款</c:if><c:if test="${item.BACKSTATE==3 }">保险费</c:if><c:if test="${item.BACKSTATE==4 }">法务费用</c:if>&nbsp;</td>
