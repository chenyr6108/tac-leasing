...
<html>
	<head>
		...
		function showEditPayMoneyReviewRecord(id,FSS_ID,CREATE_DATE){
			$("#PK").val("");
			$.ajax({
				type:'post',
				url:'../servlet/defaultDispatcher',
				data:{__action:'decompose.getFundsReturnDetailByID',
					ID:FSS_ID,CREATE_DATE:CREATE_DATE},
				dataType:'json',
	...
	<body>
		<form id="form1" name="form1" method="POST"  action="${ctx}/servlet/defaultDispatcher">
			...
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;退款付款信息管理</span>
		   	</div>
							...
							<tbody>
								<c:forEach items="${dw.resultList}" var="item" varStatus="status">												
									<tr class="ui-widget-content jqgrow ui-row-ltr">
									    ...
									    <td style="text-align: center;"><c:if test="${item.STATE==1 and auditPayMoney}"><a href="javaScript:void(0);" onclick="showEditPayMoneyReviewRecord('${item.ID }','${item.FSS_ID }','${item.CREATE_DATE }');">审批</a>&nbsp;</c:if><c:if test="${item.STATE==3 }"><%-- <a href="javaScript:void(0);" onclick="showEditPayMoney('${item.ID }','${item.FSS_ID }','${item.CREATE_DATE }');">查看(通过)</a></c:if><c:if test="${item.STATE==4 }"><a href="javaScript:void(0);" onclick="showEditPayMoney('${item.ID }','${item.FSS_ID }','${item.CREATE_DATE }');">查看(驳回)</a> --%></c:if>&nbsp;</td>
									</tr>
									<tr>
...
<div style="display:none;" id="fundsReturn" title="当前位置——退款付款审批">
	<div class="zc_grid ui-jqgrid ui-widget ui-widget-content ui-corner-all" id="main">
          <div class="zc_grid_body jqgrowleft">
			<div class="ui-widget-content jqgrow ui-row-ltr ">
				<form action="${ctx }/servlet/defaultDispatcher?__action=payMoney.updateReturnMoneyPass" id="funds_return_form" method="post">
				<input type="hidden" id="PK" name="PK" />
				<input type="hidden" id="RETURN_PAY_ID" name="RETURN_PAY_ID" />
				...
				<table cellspacing="1" cellpadding="3" border="0" class="ui-jqgrid-htable zc_grid_title">
					...
					<tr class="ui-state-default ui-jqgrid-hdiv">
						<td colspan="4" id="button_flag" style="text-align: center;">
							<input class="ui-state-default ui-corner-all" type="button" id="buttonAdd" value="通过" onclick="payMoneyPass('0')" style="cursor:pointer"></input>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="ui-state-default ui-corner-all" type="button" value="驳回" onclick="payMoneyPass('1')" style="cursor:pointer"></input>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="button" value="关&nbsp;闭" onclick="$('#fundsReturn').dialog('close');" class="ui-state-default ui-corner-all" style="cursor:pointer">
						</td>
					</tr>
				</table>
				</form>
