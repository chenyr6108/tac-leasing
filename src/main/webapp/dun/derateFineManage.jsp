...
<html>
	...
	<body>
		<form action="../servlet/defaultDispatcher" name="form1" id="form1"
			method="post">
			<c:choose>
				<c:when test="${roleFlag eq 1}">
					<input type="hidden" id="__action" name="__action"
					value="dunTask.derateFineExamine" />   
				</c:when>
				<c:otherwise>
					<input type="hidden" id="__action" name="__action"
					value="dunTask.derateFineManage" />   
				</c:otherwise>
			</c:choose>
			<input type="hidden" id="__action" name="__action"
				value="dunTask.derateFineManage" />   
					...
					<table cellspacing="0" cellpadding="0" border="0"
						...
						<c:forEach items="${dw.rs}" var="item" varStatus="status">
							<tr class="ui-widget-content jqgrow ui-row-ltr">
								...
								<td style="text-align: center"><c:choose> <c:when test="${roleFlag eq 1 && item.STATE eq 0 }"><a href="javascript:void(0)" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=dunTask.getDerateFineExamine&dufd_id=${item.DUFD_ID }'">审核|</a></c:when><c:when test="${roleFlag eq 1 && item.STATE ne 0 }"><font color="#c0c0c0">审核|</font></c:when><c:when test="${item.STATE eq 0 }"><a href="javascript:void(0)" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=dunTask.getDerateFineUpdate&dufd_id=${item.DUFD_ID }'">修改|</a></c:when><c:when test="${item.STATE eq 1 }"><a href="javascript:void(0)" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=dunTask.getDerateFineUpdate&state=1&dufd_id=${item.DUFD_ID }'">修改|</a></c:when></c:choose><a href="javascript:void(0)" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=dunTask.getDerateFineInfo&dufd_id=${item.DUFD_ID }'">详细</a></td>
							</tr> 
						</c:forEach>
						<tr>
							<td align="right" colspan="11" style="text-align:center;background-color: #C5DBEC; " class="ui-state-default ui-jqgrid-hdiv ">
