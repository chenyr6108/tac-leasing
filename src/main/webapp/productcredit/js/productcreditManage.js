...
function addProductCredit(){
	alert(111);
	// Modify by Michael 2012 08-20 协议编号改为签呈编号，且可以为空
//	if(trim(by("CUGP_CODE").value)==""){	
//		alert("协议编号为必填项");
//		$("#CUGP_CODE").focus();
//		return false;
//	}
//----Modify by Michael 2011 12/13日  增加连保、回购、回购含灭失额度的检核------
	if(trim(by("LIEN_GRANT_PRICE").value)==""){	
		alert("连保授信金额为必填项");
		$("#LIEN_GRANT_PRICE").focus();
		return false;
	}
	else
	{
		var gra = by("LIEN_GRANT_PRICE").value.replace(/\,/g,"");
		$("#LIEN_GRANT_PRICE").val(gra);
	}
	if(trim(by("REPURCH_GRANT_PRICE").value)==""){	
		alert("回购授信金额为必填项");
		$("#REPURCH_GRANT_PRICE").focus();
		return false;
	}
	else
	{
		var gra = by("REPURCH_GRANT_PRICE").value.replace(/\,/g,"");
		$("#REPURCH_GRANT_PRICE").val(gra);
	}
	
	if(trim(by("ADVANCEMACHINE_GRANT_PRICE").value)==""){	
		alert("交机前授信金额为必填项");
		$("#ADVANCEMACHINE_GRANT_PRICE").focus();
		return false;
	}
	else
	{
		var gra = by("ADVANCEMACHINE_GRANT_PRICE").value.replace(/\,/g,"");
		$("#ADVANCEMACHINE_GRANT_PRICE").val(gra);
	}
	
	$("#LIEN_LAST_PRICE").val( by("LIEN_LAST_PRICE").value.replace(/\,/g,""));
	$("#REPURCH_LAST_PRICE").val( by("REPURCH_LAST_PRICE").value.replace(/\,/g,""));
	$("#ADVANCEMACHINE_LAST_PRICE").val( by("ADVANCEMACHINE_LAST_PRICE").value.replace(/\,/g,""));
	
//	if(trim(by("REPURCHLOSS_GRANT_PRICE").value)==""){	
//		alert("回购含灭失授信金额为必填项");
//		$("#REPURCHLOSS_GRANT_PRICE").focus();
//		return false;
//	}
//	else
//	{
//		var gra = by("REPURCHLOSS_GRANT_PRICE").value.replace(/\,/g,"");
//		$("#REPURCHLOSS_GRANT_PRICE").val(gra);
//	}
//-------------------------------------------------------------------------------------------	
	...
	if(trim(by("START_DATE").value)>trim(by("END_DATE").value)){
		alert("授信开始日期必须小于授信结束日期");
		return false;
	}
...
/*	var newbox=document.getElementsByName("newbox");
	...
	*/
	if(trim(by("AUDIT").value)=="AUDIT"){
		if($("#ADVANCEMACHINE_GRANT_PRICE").val()>1000000){
			alert("审查经理的交机前拨款授信额度为100W！");
			return false;
		}
...
function delUnion(id)
{
	...
	if (confirm(msg)){
		$("#__action").val("productCredit.delUnionPrice");
		$("#DEL_UNION_ID").val(id);
		$("#formaddupd").submit();
	}
}
...
function updateUnion(id){			
				$.ajax({
					type:"post",
					url:"../servlet/defaultDispatcher",
					data:"__action=productCredit.getUnionByID&PURP_ID="+id,
					dataType:"json",
					success:function(dataBack){
				    $("#UNION_ID").val(dataBack.unionMap.PURP_ID);	
				    $("#UNION_SUPPLIER").html(dataBack.unionMap.NAME);						
				    $("#UNION_PRICE").val(dataBack.unionMap.UNION_GRANT_PRICE);												
					$('#updateUnion').dialog({
						modal:true,
						autoOpen: false							
					});
