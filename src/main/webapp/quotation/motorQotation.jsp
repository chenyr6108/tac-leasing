...
$(function (){
	if($("#init").val()=="") {
		...
		document.getElementById("yearRent").innerHTML=Number($("#totalRenPrice").val()/(${paylist.LEASE_TOPRIC==""||paylist.LEASE_TOPRIC==null?1:paylist.LEASE_TOPRIC }
		-${paylist.PLEDGE_AVE_PRICE==""||paylist.PLEDGE_AVE_PRICE==null?1:paylist.PLEDGE_AVE_PRICE})/($("#PAY_LEASE_PERIOD").val()==""?12:$("#PAY_LEASE_PERIOD").val()/12)*100).toFixed(2)+'%';	
		$("#hanshui").val(Number($("#totalRenPrice").val()/(${paylist.LEASE_TOPRIC==""||paylist.LEASE_TOPRIC==null?1:paylist.LEASE_TOPRIC }
		-${paylist.PLEDGE_AVE_PRICE==""||paylist.PLEDGE_AVE_PRICE==null?1:paylist.PLEDGE_AVE_PRICE})/($("#PAY_LEASE_PERIOD").val()==""?12:$("#PAY_LEASE_PERIOD").val()/12)*100).toFixed(2)+'%');
	}
});
...
/* 			    if(_sum.toFixed(2)!=(_ave+_back+_last).toFixed(2)){
			    ...
			    /* if(_sum<=0){
				  ...
				  if (flag == 1){
					  form.__action.value = "quotation.saveScheme1";
					  form.submit();
				  }else if (flag == 2) {
...
<form action="../servlet/defaultDispatcher" name="payForm" id="payForm" method="post">
	...
	<table width="900" cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="wuzd">
		...
		<tr>
			...
			<td   style="text-align:  left" class="ui-widget-content jqgrow ui-row-ltr">
				<input id="pay__money" onkeyup="value=value.replace(/[^\d]/g,'')" onchange="paylistchangeRZE()" value="${empty pay__money?0:pay__money }" name="pay__money">
			</td>
		</tr>
		<tr>
			<td   class="ui-widget-content jqgrow ui-row-ltr">
				手续费收入：<input type="text" id="incomePay" name="incomePay" value="${incomePay }" style="width: 60px;" onchange="changeRze1()">
			</td>
			<td  class="ui-widget-content jqgrow ui-row-ltr">
				手续费收入税额：<input type="text" id="incomePayTax" name="incomePayTax" value="${empty incomePay?0:incomePay*0.055 }" style="width: 60px;" readonly="readonly">
			</td>
			<td class="ui-widget-content jqgrow ui-row-ltr">
				银行手续费：<input type="text" id="outPay" name="outPay" value="${outPay }" style="width: 60px;" onchange="paylistchangeRZE()">
			</td>
		...
		<tr>
			...
			<td   style="text-align:  left" class="ui-widget-content jqgrow ui-row-ltr">
				<input type="text" onchange="changeFieldPay1('PAY_PLEDGE_PRICE_RATE',this);"
					...
					value="${empty paylist.PLEDGE_PRICE_RATE?0:paylist.PLEDGE_PRICE_RATE }" style="width: 60px;">
				%
				<input type="text" onchange="changeFieldPay1('PAY_PLEDGE_PRICE',this);"
					name="PAY_PLEDGE_PRICE" id="PAY_PLEDGE_PRICE"
					value="<fmt:formatNumber value="${empty paylist.PLEDGE_PRICE?0:paylist.PLEDGE_PRICE }" type="currency" />"
					class="input_right" style="width: 130px;">
			</td>
			<td  class="ui-widget-content jqgrow ui-row-ltr">
...
<!-- Add by Michael 2012 01/14 费用参数化 -->
		...
		<tr>
			<td  class="ui-widget-content jqgrow ui-row-ltr" colspan="4">
				...
				年平息率：<span id="yearRent">NaN %</span><input id="hanshui" name="hanshui" type="hidden">&nbsp;&nbsp;
			</td>
		</tr>
		<tr>
	...
	<center>
	<input type="hidden" name="REN_PRICE" id="REN_PRICE" value="${REN_PRICE }"	class="input_right" style="width: 130px;" readonly="readonly">
