...
<form action="../servlet/defaultDispatcher" name="payForm" id="payForm" method="post">
	...
	<input type="hidden" name="SCHEME_ID" value="${paylist.SCHEME_ID }">
		...
		<tr>
			...
			<td style="text-align: left" class="ui-widget-content jqgrow ui-row-ltr">
				...
				<select name="IS_TAX" id="IS_TAX" onchange="changeLeaseAllPrice();">
				 	<option value="0" <c:if test="${paylist.IS_TAX eq 0 }">selected="selected" </c:if>>含税</option>
					<option value="1" <c:if test="${paylist.IS_TAX eq 1 }">selected="selected" </c:if>>未税</option>
				</select>
			</td>
...
<!-- Add by Michael 2012 01/14 费用参数化 -->
		...
		<tr>
			<td colspan="4" style="text-align: left;height: 26px;" class="ui-widget-content jqgrow ui-row-ltr">备注：
				<textarea rows="" cols="" style="height: 65px;width: 500px" name="REMARK"></textarea>
				&nbsp;
			</td>
		</tr>		
		<tr>
