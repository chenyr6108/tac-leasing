...
<html>
	<head>
		...
		function examSettleFile(TYPE,ID){	
			var alterStr="";
			if (TYPE==1){
				alterStr="确认通过此次申请！";
			}else if(TYPE==2){
			...
			if(confirm(alterStr)){
				...
				var data = {"__action" : "rentContract.upDateRentSettleFileExam", "TYPE" : TYPE,"ID":ID};
				$.getJSON(url, data, function (result){
					if(result){
						location.href = '${ctx }/servlet/defaultDispatcher?__action=rentContract.queryRentSettleFileExam';
					} else {
						alert("操作失败。");
					}
				});
...
<tr id="cz001" class="ui-widget-content jqgrow ui-row-ltr">
	...
	<tr class="ui-widget-content jqgrow ui-row-ltr">
       ...
       <tr class="ui-jqgrid-labels">
		...
		<c:if test="${rentSettleFileApply.STATE eq 0}">
			<input class="ui-state-default ui-corner-all" id="buttonAdd" type="button" value="通过" onclick="examSettleFile('1','${rentSettleFileApply.ID}');" style="cursor: pointer;"/> &nbsp;&nbsp;&nbsp;&nbsp;<input class="ui-state-default ui-corner-all" id="buttonAdd" type="button" value="驳回" onclick="examSettleFile('2','${rentSettleFileApply.ID}');" style="cursor: pointer;"/>
		</c:if>
		</td>							
	</tr>															
		</table>				
