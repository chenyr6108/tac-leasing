    ...
    $("#POST_DATE").datepicker($.datepicker.regional['zh-CN']);
		...
		function subRentFile(){
			...
			$('input[name="rent_ids"]:checked').each(function(){  
				...
				rentForm.file_names.value=rent_ids;
...
<body>
					...
					<table cellspacing="0" cellpadding="0" border="0"
						...
						<c:forEach items="${insorupd}" var="item" varStatus="status">
							...
							<tr id="cz001" class="ui-widget-content jqgrow ui-row-ltr">
								<td width="8%"><INPUT TYPE="checkbox" NAME="ids" 
									value="${item.REFI_ID},${item.FILE_NAME}" style="width: 100%;"<c:if test="${item.POST_FLAG eq 1}">checked</c:if> disabled></td>
								...
								<td width="9%"><input type="text"
									...
									style="width: 100%; border-top-style: none; border-left-style: none; border-right-style: none; border-bottom-style: none" disabled></td>
	...
	<tr class="ui-widget-content jqgrow ui-row-ltr">
		...
		<td style="text-align: left;" colspan="5"><c:if test="${postMap.POST_DATE !='1900-01-01'}">${postMap.POST_DATE }</c:if>&nbsp;</textarea></td>							
	</tr>
	<tr class="ui-widget-content jqgrow ui-row-ltr">
		<td style="text-align: center;width:7%;">快递公司</td>
		<td style="text-align: left;" colspan="5">${postMap.POST_NAME }&nbsp;</textarea></td>							
	</tr>		
	<tr class="ui-widget-content jqgrow ui-row-ltr">
		<td style="text-align: center;width:7%;">寄件单号</td>
		<td style="text-align: left;" colspan="5">${postMap.POST_CODE }&nbsp;</textarea></td>							
	</tr>						
	<tr class="ui-widget-content jqgrow ui-row-ltr">
		<td style="text-align: center;width:7%;">备注说明：</td>
		<td style="text-align: left;" colspan="5">${postMap.MEMO }</textarea></td>							
	</tr>						
						<tr class="ui-jqgrid-labels">
