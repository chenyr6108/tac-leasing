...
<html>
	...
	<body>
		<form id="form1" name="form1" method="POST"  action="${ctx}/servlet/defaultDispatcher">
			<input type="hidden" name="__action" id="__action" value="rentContract.queryRentContractByPlay" />
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;新建锁码</span>
		   	</div>
					...
					<select name="RENTSTAUTS" style=" height: 18px;">
				          ...
				          <tr>
					          ...
					          <td colspan="3"><input type="text" name="QSEARCH_VALUE" value="${QSEARCH_VALUE }" 
					          	style=" width:230px;height:18px; font-size: 12px;">
					          </td>
					          <td>机号：</td>
					          <td><input type="text" name="search_thing_number" value="${search_thing_number }"/></td>
				          </tr>
				          </table>
							...
							<tbody>
								<c:forEach items="${dw.resultList}" var="item" varStatus="status">													
									<tr class="ui-widget-content jqgrow ui-row-ltr">
										<td style="text-align: center;" onClick="expandForShowByPlay('pactAffix${item.PRCD_ID}','${item.PRCD_ID }','${item.RECT_ID }','manage','${item.STATUS }');" style="cursor: pointer;"><!-- 	[${item.PRCD_STATUS }|${item.RECT_STATUS }] --><c:if test="${item.RECT_STATUS eq null}"><img src="${ctx }/images/u608.gif" title="未初审"></c:if><c:if test="${item.STATUS eq 0}"><c:if test="${item.RECT_STATUS eq 0}"><img src="${ctx }/images/u622.gif" title="已初审"></c:if><c:if test="${item.RECT_STATUS eq 1}"><img src="${ctx }/images/u611.gif" title="已复核"></c:if><c:if test="${item.RECT_STATUS eq 2}"><img src="${ctx }/images/u617.gif" title="复核驳回"></c:if></c:if><c:if test="${item.STATUS eq -1}"><img src="${ctx }/images/u620.gif" title="作废"></c:if></td>
