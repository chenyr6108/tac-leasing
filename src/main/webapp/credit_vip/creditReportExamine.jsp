...
<html>
	...
	function creditfile(id) {
		$("#creditfile").toggle(
				function() {
					$("#creditfile").load("../servlet/defaultDispatcher?__action=creditReportVip.queryRentFile&prcd_id="+id+"&cardFlag=0"+"&rentFileFlag=2");
				});	
		$('#creditfile').dialog({
			modal:true,
			autoOpen: false,
	...
	<body>
		<form action="../servlet/defaultDispatcher" name="form1" id="form1" method="post">
			<input type="hidden" name="__action" id="__action" value="creditReportVip.creditExamine">
			...
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;报告审批(绿色通道)</span>
		   	</div>
			...
			<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
				<tr class="ui-jqgrid-labels">
					...
					<th class="ui-state-default ui-th-ltr zc_grid_head">
						<page:pagingSort orderBy="CLERK_NAME" pagingInfo="${pagingInfo }"><strong>服务课人员 </strong></page:pagingSort>
					</th>
					<th class="ui-state-default ui-th-ltr zc_grid_head">
				...
				<c:forEach items="${pagingInfo.resultList}" var="item">
					<tr class="ui-widget-content jqgrow ui-row-ltr">
						...
						-->
						<td style="text-align: center;"><a href="javaScript:void(0)"	onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=creditReportVip.selectCreditForShow&credit_id=${item.ID }'">查看</a>|<c:choose><c:when test="${item.STATE eq 3 and item.STATUS eq 0}"><a href="javaScript:void(0)"	onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=creditReportVip.selectCreditForShow&credit_id=${item.ID }&examineFlag=1'">审批</a></c:when><c:otherwise><font color="#c0c0c0">审批</font></c:otherwise></c:choose><%--作废启用废除,加入到业务支撑 |<c:if test="${item.STATUS eq 0}"><c:choose><c:when test="${item.STATE eq 3}"><a href="javaScript:void(0)" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=creditReport.invCredit&credit_id=${item.ID }'">作废</a></c:when><c:otherwise><font color="#c0c0c0">作废</font></c:otherwise></c:choose></c:if><c:if test="${item.STATUS eq -1}"><c:choose><c:when test="${item.STATE eq 3}"><a href="javaScript:void(0)" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=creditReport.enaCredit&credit_id=${item.ID }'">启用</a></c:when><c:otherwise><font color="#c0c0c0">启用</font></c:otherwise></c:choose></c:if> --%>|<a href="javascript:void(0)" onclick="creditfile('${item.ID}');">资料</a> </td>
