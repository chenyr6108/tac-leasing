...
//		data:'__action=employee.getZhuguan',
//		dataType:'json',
...
function addEmployee(dept_count){
	if(trim(by("name").value)==""||trim(by("code").value)==""||trim(by("password").value)==""||trim(by("password2").value)==""){
		alert("请填写必填项！");
		return false;
	}
	if($("#id_cardAge").val()=="未知"||$("#id_cardAge").val()=="")
	{
		alert("请填写年龄");
		return false;
	}
	if(positive($("#id_cardAge")[0],'年龄')){
		return false;
	}
...
//作废一条员工记录
function invalidEmployee(id){
	if(confirm('确认作废该条记录？')){
		location.href='../servlet/defaultDispatcher?__action=employee.invalid&id='+id;
	}else{
		return false;
	}
}
