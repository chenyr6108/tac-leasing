...
showCust = function (id){
	$("#__action").val("custCredit.getCustCreditJsp");
	$("#cust_id").val(id);
	$("#CUST_ID").val(id);
	$("#form1").submit();
	$("#__action").val("custCredit.queryAllCustCredit");
	$("#cust_id").val("");
	$("#CUST_ID").val("");

}
function customerShowSearch(){
	 $("#res").val("cs");
	 doSearch();  
}
function addCustRow(){
	    ...
	    cell.className = "ui-widget-content jqgrow ui-row-ltr";
	    cell.height= '26px';
	    cell.innerHTML ='<select name="PAY_WAY1" id="PAY_WAY1"><option value="">---请选择---</option><option value="11">期初等额本息支付</option><option value="12">期初等额本金支付</option><option value="13">期初不等额支付</option><option value="21">期末等额本息支付</option><option value="22">期末等额本金支付</option><option value="23">期末不等额支付</option></select>';
...
function delCreditPlan(id)
{
	var msg = "您确定要取消授信？";
	if (confirm(msg)){
		$("#__action").val("custCredit.delCustCreditPlan");
		$("#cust_id").val(id);
		$("#CUST_ID").val(id);
		$("#form1").submit();
	}
...
showLog = function (id){
	$("#__action").val("custCredit.queryLog");
	$("#cust_id").val(id);
	$("#CUST_ID").val(id);
	$("#form1").submit();
}
