	...
	<body>
		<form id="form1" name="form1" method="POST" action="${ctx}/servlet/defaultDispatcher">
			...
			<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		   		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;客户授信浏览</span>
		   	</div>
				...
				<div style="margin: 6 0 6 0px;">
	          	<table width="97%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
				      ...
				      <td width="8">&nbsp;</td>
				        ...
				        <td width="20%"><a href="#" name="search" id="search" onclick="customerShowSearch();"  class="blue_button">搜 索</a></td>
				      </tr>
				    </table>
	          		</div>
		...
		<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="tableForColor">
			<thead>
				<tr class="ui-jqgrid-labels">
					...
					<th class="ui-state-default ui-th-ltr zc_grid_head" style="width: 300px">
						客户
					</th>
			...
			<c:forEach items="${dw.rs}" var="item" varStatus="status">
					...
					<td style="text-align: center;">${item.CUST_CODE }&nbsp;<input type="hidden" name="custid" id="custid" value="${item.CUST_ID }"/></td>
					<td style="text-align: left;"><a href="javaScript:void(0)" onclick="window.parent.openTab({tabName:'客户信息浏览',location:'${ctx }/servlet/defaultDispatcher?__action=customer.readCust&cust_id=${item.CUST_ID }&cust_type=${item.CUST_TYPE }&showFlag=0'});">${item.CUST_NAME }</a>&nbsp;<input type="hidden" name="custname" id="custname" value="${item.CUST_NAME }"/></td>
					<td style="text-align: center;"><c:forEach items="${custInfor}" var="custInfor" varStatus="sta"><c:if test="${item.CUST_LEVEL eq custInfor.CODE}">${custInfor.FLAG}</c:if></c:forEach>&nbsp;</td>
					...
					<td style="text-align: center;">${item.NATU_PHONE }<c:if test="${empty item.NATU_PHONE }">${item.NATU_MOBILE }</c:if> &nbsp;</td>
					<td style="text-align: center;"><c:if test="${empty item.CUGP_ID}">未授信</c:if><c:if test="${item.CUGP_STATUS eq 0}">已授信</c:if><c:if test="${item.CUGP_STATUS eq 1}">取消授信</c:if></td>
					<td style="text-align: center;"><a href="#" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=custCredit.queryLog&CUST_ID=${item.CUST_ID }'">查看</a>&nbsp;</td>
				</tr>
			</c:forEach>
			</tbody>
		</table>
