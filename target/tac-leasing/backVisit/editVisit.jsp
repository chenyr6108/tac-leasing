...
$(function(){
    ...
    $("#addForm").val("${pageContext.request.contextPath}/servlet/defaultDispatcher?__action=backVisit.updateVisitManagerByVisit_Id");
    $("#addForm").submit();
...
<form action="${pageContext.request.contextPath}/servlet/defaultDispatcher?__action=backVisit.updateVisitManagerByVisit_Id" method="post" id="addForm" name="addForm" >
	...
	<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
		<tr id="1" class="ui-widget-content jqgrow ui-row-ltr">
			<input type="hidden" name="visit_id" id="visit_id" value="${backVisit.VISIT_ID }">
			<td>合同编号：</td>
		...
		<tr id="3" class="ui-widget-content jqgrow ui-row-ltr">
			...
			<td><input type="text" style="height:25px" name="visitNewDate" id="datebegin" value="${backVisit.VISIT_DATE }"/></td>
			<td>是否营业： </td>
			<td><select name="is_business" id="is_business"><option value="0" <c:if test='${backVisit.IS_BUSINESS eq 0 }'>selected="selected"</c:if>>是</option><option value="1" <c:if test='${backVisit.IS_BUSINESS eq 1 }'>selected="selected"</c:if>>否</option></select></td>
			...
			<td>是否见标的物： </td>
			<td><select name="is_products" id="is_products"><option value="0" <c:if test='${backVisit.IS_PRODUCTS eq 0 }'>selected="selected"</c:if>>是</option><option value="1" <c:if test='${backVisit.IS_PRODUCTS eq 1 }'>selected="selected"</c:if>>否</option></select></td>
		</tr>
		<tr id="3" class="ui-widget-content jqgrow ui-row-ltr">
			<td>标的物外观： </td>
