...
function expand(FIIN_ID){	
	...
	<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
		<tr class="ui-jqgrid-labels">
			...
			<!-- Add by michael 2011 12/12 增加分解反馈信息，比对来款是否与要分解的款项一致 -->
			<th width="" class="ui-state-default ui-th-ltr zc_grid_head">
				<strong>分解反馈</strong>
			</th>			
			<th width="" class="ui-state-default ui-th-ltr zc_grid_head">
		...
		<c:forEach items="${dw.rs}" var="income" varStatus="status">
			<tr	class="ui-widget-content jqgrow ui-row-ltr">		
				...
				<td style="text-align: center;" <c:if test="${income.QTY >0 }">onclick="expand('${income.FIIN_ID}');"</c:if>>&nbsp;<c:if test="${income.QTY >0 }"><img src="${ctx }/images/attach.jpg" title="已有水单上传" style="cursor: pointer;">${income.QTY }份</c:if></td>			
				<td style="text-align: center;">${income.MAIL_MEMO}&nbsp;</td>
				...
				<!-- Add by michael 2011 12/12 增加分解反馈信息，比对来款是否与要分解的款项一致 -->
				<td style="text-align: center;"><c:choose><c:when test="${income.ABNORMAL_STATUS eq 1}"><img src="${ctx }/images/u604.gif"></c:when><c:when test="${income.ABNORMAL_STATUS eq 0}"><img src="${ctx }/images/u622.gif"></c:when></c:choose></td>
				...
				<td style="text-align: center;">${income.OPPOSING_UNIT }<input type="hidden" name="opposing_name" id="opposing_name${status }" value="${income.OPPOSING_UNIT }" />&nbsp;</td>
	 			<td style="text-align: center;">${income.OPPOSING_UNIT_TRUE }&nbsp;</td>
				...
				<td style="text-align: center;">${income.VIRTUAL_CODE }&nbsp;</td>
				...
				<td style="text-align: center;">
					<c:if  test ="${not empty income.OPPOSING_MEMO}">
						<a href="#" onmouseover="mouseOver(event, '${income.OPPOSING_MEMO}')" onmouseout="mouseOver(event, 0)">显示</a>
					</c:if> &nbsp; 
				</td>
				<td style="text-align: center;">
						<c:choose>
							<c:when test="${not empty income.OPERATE_ID}">
								<a href="javascript:showRemarkDiv('${income.FIIN_ID }','1','${income.OPERATE_REMARK }  --  ${income.OPERATE_NAME }  ${income.OPERATE_TIME }↓')">查看</a>&nbsp;<a href="javascript:showRemarkDiv('${income.FIIN_ID }','2','${income.OPERATE_REMARK }  --  ${income.OPERATE_NAME }  ${income.OPERATE_TIME }↓')">修改</a>
							</c:when>
							<c:otherwise>
								<a href="javascript:showRemarkDiv('${income.FIIN_ID }','0','')">添加</a>
							</c:otherwise>
						</c:choose>
				</td>
			</tr>
			...
			<tr>
				<td style="text-align: center;" colspan="10"  align="center">
					<div id="pactAffix${income.FIIN_ID}" align="center" style="margin-left:15px ;margin-right:15px;margin-top:10px;margin-bottom:20px;  display: none; width: 40%">
			...
			<tr>
				<td style="text-align: center;" colspan="10" align="center">
					<div id="pactAffix_${income.FIIN_ID}" align="center"
