									...
									<input type="hidden" id="CUST_NAME_R" name="CUST_NAME_R" value="${rentCheckDetail.CUST_NAME }" />
										...
										<td colspan="7">			
											...
											class="ui-state-default ui-th-ltr zc_grid_head">操作&nbsp;&nbsp;&nbsp;&nbsp;
													<c:choose>
														<c:when test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">
														</c:when>
														<c:otherwise>
										...
										<c:forEach items="${rentCheckLeaseCodeList}" var="item" varStatus="status">
											<tr>
												<td class="ui-widget-content jqgrow ui-row-ltr"><input type="text" id="CUST_NAME${ status.count }"	name="CUST_NAME" size="30" onblur="getCustRentLeaseCode(START_PERIOD_NUM${ status.count },END_PERIOD_NUM${ status.count },LEASE_CODE${ status.count },this.value,this);" value="${item.CUST_NAME }" <c:if test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;<input type="hidden" id="CHECK_ID${ status.count }"	name="CHECK_ID_R" value="${item.CHECK_ID }"></td>
												<td class="ui-widget-content jqgrow ui-row-ltr">
													<select id="LEASE_CODE${ status.count }" name="LEASE_CODE" onchange="loadLeaseTerm(START_PERIOD_NUM${ status.count },END_PERIOD_NUM${ status.count },this.value);" <c:if test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
												...
												<td class="ui-widget-content jqgrow ui-row-ltr">
													<select id="START_PERIOD_NUM${ status.count }" name="START_PERIOD_NUM" <c:if test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
												...
												<td class="ui-widget-content jqgrow ui-row-ltr">
													<select id="END_PERIOD_NUM${ status.count }" name="END_PERIOD_NUM" <c:if test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
											...
											<tr>
												<td colspan="7" align="center"
													class="ui-widget-content jqgrow ui-row-ltr"><input class="ui-state-default ui-corner-all" id="buttonAdd" type="button" value="保存数据" onclick="subCheckPayLines();" style="cursor: pointer;" <c:if test="${not empty rentCheckDetail.FINANCE_CHECK_STATE and rentCheckDetail.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/></td>
											</tr>
								</table>
							</div>
						</div>
