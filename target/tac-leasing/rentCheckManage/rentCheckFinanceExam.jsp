...
/* 	    var checkOutDates = document.getElementsByName("CHECKOUT_DATE");
		...
		checkPayForm.all_check_ids.value=check_ids;
								...
								<table cellspacing="0" cellpadding="0" border="0"
									...
									<c:choose>
										...
										<c:forEach items="${checkManagePayLines }" var="item"
												...
												<input type="hidden" id="MONTH_PRICE_TAX" name="MONTH_PRICE_TAX" value="${item.MONTH_PRICE_TAX}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>
												<tr>
													<td style="text-align: center"
														class="ui-widget-content jqgrow ui-row-ltr">第${item.PERIOD_NUM}期&nbsp;<input type="hidden" id="PERIOD_NUM" name="PERIOD_NUM" value="${item.PERIOD_NUM}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/><input type="hidden" id="ID" name="ID" value="${item.ID}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/></td>
													<td style="text-align: right; height: 24px;"
														class="ui-widget-content jqgrow ui-row-ltr">
														<fmt:formatNumber
															value="${item.MONTH_PRICE_TAX }" type="currency" />&nbsp;</td>
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="DRAWER" value="${item.DRAWER}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>			
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECKOUT_DATE" class="DATE_SPI" style=" width: 80px; height: 18px;" readonly="readonly" value="<c:choose><c:when test="${item.CHECK_OUT_DATE eq '1900-01-01'}"></c:when><c:otherwise>${item.CHECK_OUT_DATE}</c:otherwise></c:choose>" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>&nbsp;</td>
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECK_NUM" value="${item.CHECK_NUM}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><input type="text" name="CHECK_MONEY" value="${item.CHECK_MONEY}" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><select name="MEMO" id="MEMO" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>>
														<c:forEach items="${memoList}" var="item1">
																	<option value="${item1.FLAG }" <c:if test="${item1.FLAG eq item.MEMO}">selected="selected"</c:if>>
																		${item1.FLAG }
																	</option>
														</c:forEach></select>&nbsp;</td>
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><INPUT TYPE="checkbox" NAME="check_ids_hw" id="check_ids_hw" value="${item.ID}" style="width:100%;" <c:if test="${not empty item.HW_CHECK_STATE and item.HW_CHECK_STATE eq 1}">checked</c:if> disabled/>&nbsp;</td>				
													<td style="text-align: center;"class="ui-widget-content jqgrow ui-row-ltr"><INPUT TYPE="checkbox" NAME="check_ids" id="check_ids" value="${item.ID}" style="width:100%;" <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">checked</c:if> <c:if test="${not empty item.FINANCE_CHECK_STATE and item.FINANCE_CHECK_STATE eq 1}">disabled</c:if>/>&nbsp;</td>
												</tr>
											</c:forEach> 
