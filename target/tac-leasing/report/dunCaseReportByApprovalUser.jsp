...
function expand(iddiv,approvalUserId,approvalUserName,date){
    $("#"+iddiv).toggle(function(){
    	var urlParams = "dun_date___"+ date + "@@approvalUserName___" + approvalUserName;
        $(this).load("../servlet/defaultDispatcher?__action=dunCaseCommand.queryDunCaseReportDetail&DATE="+date+"&approvalUserId="+approvalUserId+"&type="+$('input[name="type"]:checked').val()+"&byType="+$('#byType').val()+"&urlParams="+urlParams);
    });
}
function queryDetail(approvalUserName,startrange,endrange,date){
	window.location = "../servlet/defaultDispatcher?__action=dunTask.manage&dun_date="+date+"&approvalUserName="+approvalUserName+"&startrange="+startrange+"&endrange="+endrange;
}
</script>
</head>
<body>
	<form action="../servlet/defaultDispatcher" method="post">
		<input type="hidden" id="byType" name="byType" value="approvalUser" />
		<input type="hidden" name="__action"  value="dunCaseCommand.queryDunCaseReport" />
		<div style="font-size:24px; font-weight:bold; color:#2E6E9E; text-align:center;">逾期状况统计表6（按核准人）</div>
		...
		<div id="type" style="margin-top:4px;">分类
			<input type="radio" value="dept" id="radio1" name="type" <c:if test="${type=='dept'||(type!='orgUpUser'&&type!='orgUser'&&type!='price'&&type!='special') }">checked="checked"</c:if> /><label for="radio1">办事处</label>
			<input type="radio" value="special" id="radio2" name="type" <c:if test="${type=='special' }">checked="checked"</c:if> /><label for="radio2">专案</label>
			<input type="radio" value="orgUpUser" id="radio3" name="type" <c:if test="${type=='orgUpUser' }">checked="checked"</c:if> /><label for="radio3">原始主管</label>
			<input type="radio" value="orgUser" id="radio4" name="type" <c:if test="${type=='orgUser' }">checked="checked"</c:if> /><label for="radio4">原始经办人</label>
			<input type="radio" value="price" id="radio5" name="type" <c:if test="${type=='price' }">checked="checked"</c:if> /><label for="radio5">金额</label>
		</div>
	</form>
					...
					<tr>
						<c:if test="${item.count1_7 != null }">
							<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center; white-space:nowrap;">
								<a href="javascript:void(0);" onclick="expand('pactAffix${index.count}','${item.approvalUserId  }','${item.approvalUserName  }','${DATE }')">${item.approvalUserName  }</a>
								&nbsp;
							</td>
						</c:if>
						<c:if test="${item.count1_7 == null }">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count8_14!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',8,14,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count15_30!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',15,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;">
							...
							<c:if test="${item.count31_60!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',31,60,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count61_90!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',61,90,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',8,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',31,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',31,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',91,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count91_180)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',91,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.countMoreThan181!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',181,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30+item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.approvalUserName}',8,'','${DATE }');">
	...
	<p align="right" style="font-weight:bolder;">编号：IT-RPT-27</p>
	<script>
	$(function () {
		var d=new Date();
		if($("#DATE").val() != ''){
