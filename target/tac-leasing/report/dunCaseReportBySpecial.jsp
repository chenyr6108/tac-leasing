...
function expand(iddiv,CREDIT_SPECIAL_CODE,date){
    $("#"+iddiv).toggle(function(){
    	var urlParams = "dun_date___"+ date + "@@CREDIT_SPECIAL_CODE___" + CREDIT_SPECIAL_CODE;
        $(this).load("../servlet/defaultDispatcher?__action=dunCaseCommand.queryDunCaseReportDetail&DATE="+date+"&CREDIT_SPECIAL_CODE="+CREDIT_SPECIAL_CODE+"&type="+$('input[name="type"]:checked').val()+"&byType="+$('#byType').val()+"&urlParams="+urlParams);
    });
}
function queryDetail(CREDIT_SPECIAL_CODE,startrange,endrange,date){
	window.location = "../servlet/defaultDispatcher?__action=dunTask.manage&dun_date="+date+"&CREDIT_SPECIAL_CODE="+CREDIT_SPECIAL_CODE+"&startrange="+startrange+"&endrange="+endrange;
}
</script>
</head>
<body>
	<form action="../servlet/defaultDispatcher" method="post">
		<input type="hidden" id="byType" name="byType" value="special" />
		<input type="hidden" name="__action"  value="dunCaseCommand.queryDunCaseReport" />
		<div style="font-size:24px; font-weight:bold; color:#2E6E9E; text-align:center;">逾期状况统计表2（按专案名）</div>
		...
		<input type="submit" value="查询" style="width:50px; height:30px; margin-left:10px; pointer;" class="ui-state-default ui-corner-all"/>
	 	<div id="type" style="margin-top:4px;">分类
			<input type="radio" value="dept" id="radio1" name="type" <c:if test="${type=='dept'||(type!='orgUpUser'&&type!='orgUser'&&type!='price') }">checked="checked"</c:if> /><label for="radio1">办事处</label>
				...
				<c:forEach items="${resultList }" var="item" varStatus="index">
					...
					<c:if test="${item.count1_7 != null }">
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center; white-space:nowrap;">
							<a href="javascript:void(0);" onclick="expand('pactAffix${index.count}','${item.creditSpecialCode  }','${DATE }')">${item.creditSpecialCode=="-1"?"非专案":item.creditSpecialName  }</a>
							&nbsp;
						</td>
					</c:if>
					<c:if test="${item.count1_7 == null }">
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							${item.creditSpecialCode=="-1"?"非专案":item.creditSpecialCode }
						</td>
					</c:if>
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count8_14!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',8,14,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count15_30!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',15,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;">
							...
							<c:if test="${item.count31_60!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',31,60,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count61_90!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',61,90,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',8,30,'${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',31,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count31_60+item.count61_90+item.count91_180)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',31,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',91,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.count91_180!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',91,'180','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${item.countMoreThan181!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',181,'','${DATE }');">
						...
						<td class="ui-widget-content jqgrow ui-row-ltr" style="text-align: center;"> 
							...
							<c:if test="${(item.count8_14+item.count15_30+item.count31_60+item.count61_90+item.count91_180+item.countMoreThan181)!=0}">
								<a href="javascript:void(0);" onclick="queryDetail('${item.creditSpecialCode}',8,'','${DATE }');">
	...
	<p align="right" style="font-weight:bolder;">编号：IT-RPT-23</p>
	<script>
	$(function () {
		var d=new Date();
		if($("#DATE").val() != ''){
