...
function changeTax() {
	if($("#PAY_LEASE_ALLPRIC").val().replace(/[￥,]/g, '')-$("#PAY_PLEDGE_PRICE").val().replace(/[￥,]/g, '')<100000) {
		$("#incomePay").val(2000);
	 ...
	 function addIrrMonthPriceRowX(){
		...
		function submitQuotation(flag) {
		    ...
		    if (form.PAY_LEASE_TERM.value == 0) {
			    ...
			    if (notPositive(form.STAYBUY_PRICE, '注销抵押设定费用')) {
			        return;
			    }
		...
		function changePledgeLastPriceNew(){
...
<form action="../servlet/defaultDispatcher" name="payForm" id="payForm"  method="post">
	...
	<input type="hidden" name="CONTRACT_TYPE" value="8">
	...
	<table width="900" cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="wuzd">
		...
		<tr>
			...
			<td style="text-align: left" class="ui-widget-content jqgrow ui-row-ltr">
				<input type="text" name="PAY_LEASE_ALLPRIC" id="PAY_LEASE_ALLPRIC" onchange="changeTax();changeLeaseAllPrice();"
		...
		<tr>
			...
			<td width="25%"style="text-align:  left" class="ui-widget-content jqgrow ui-row-ltr">
				<input type="text" onchange="updateYearInterest();jsPayListAddItemToSelect();changeFieldPay('PAY_LEASE_PERIOD',this);"
	...
	<tr>
			...
			<td   class="ui-widget-content jqgrow ui-row-ltr">
				业务员佣金<font color="red">&nbsp;*</font>
			</td>
			<td   style="text-align:  left" class="ui-widget-content jqgrow ui-row-ltr">
				<input id="pay__money" onkeyup="value=value.replace(/[^\d]/g,'')" onchange="paylistchangeRZE()" value="${schema.SALES_PAY }" name="pay__money">
			</td>
		</tr>
		<tr>
			<td   class="ui-widget-content jqgrow ui-row-ltr">
				手续费收入：<input type="text" id="incomePay" name="incomePay" value='<fmt:formatNumber value="${schema.INCOME_PAY }" pattern="##0.00"/>' readonly="readonly" style="width: 60px;">
			</td>
			<td  class="ui-widget-content jqgrow ui-row-ltr">
				手续费收入税额：<input type="text" id="incomePayTax" name="incomePayTax" value='<fmt:formatNumber value="${schema.INCOME_PAY*0.055 }" pattern="##0.00"/>' readonly="readonly" style="width: 60px;">
			</td>
			<td class="ui-widget-content jqgrow ui-row-ltr">
				银行手续费：<input type="text" id="outPay" name="outPay" value='<fmt:formatNumber value="${schema.OUT_PAY }" pattern="##0.00"/>' readonly="readonly" style="width: 60px;">
			</td>
		...
		<tr>
			...
			<td  class="ui-widget-content jqgrow ui-row-ltr" style="font-size: 14px;" colspan="3">
				<select name="PAY_TAX_PLAN_CODE" id="PAY_TAX_PLAN_CODE" onchange="hiddenButton()">
				...
				<input type="hidden" name="TOTAL_VALUEADDED_TAX" id="TOTAL_VALUEADDED_TAX" value="${ empty schema.TOTAL_VALUEADDED_TAX?0:schema.TOTAL_VALUEADDED_TAX}"/>
			</td>
		</tr>
		<tr>
			<td colspan="4">
		...
		<tr>
			...
			<td   style="text-align:  left" class="ui-widget-content jqgrow ui-row-ltr">
				<input type="text" onchange="changeTax();changeFieldPay1('PAY_PLEDGE_PRICE_RATE',this);"
		...
		<tr>
					<td  class="ui-widget-content jqgrow ui-row-ltr">
						用于平均抵冲租金 <input type="hidden" name="PAY_PLEDGE_REALPRIC" id="PAY_PLEDGE_REALPRIC" value="0"/>
					</td>
					<td  class="ui-widget-content jqgrow ui-row-ltr">
				...
				<tr>
					...
					<td  class="ui-widget-content jqgrow ui-row-ltr">
						...
						<select name="PAY_PLEDGE_LAST_PERIOD" onchange="changePledgeLastPriceNew();" id="PAY_PLEDGE_LAST_PERIOD">
							<option value="0" <c:if test="${empty schema.PLEDGE_LAST_PERIOD and schema.PLEDGE_LAST_PERIOD eq 0}"> selected="selected"</c:if>>0</option>
							<c:forEach var="i" begin="1" end="${schema.LEASE_PERIOD }" step="1">
							  <option value="${i }" <c:if test="${not empty schema.PLEDGE_LAST_PERIOD and schema.PLEDGE_LAST_PERIOD == i}"> selected="selected"</c:if>>${i}</option>
							</c:forEach>
						</select> 
						<input type="hidden" name="PLEDGE_LAST_PRICE_TAX" id="PLEDGE_LAST_PRICE_TAX" value="${empty schema.PLEDGE_LAST_PRICE_TAX?0: schema.PLEDGE_LAST_PRICE_TAX}"/>					
					</td>
...
<!-- Add by Michael 2012 01/14 费用参数化 -->
		...
		<tr>
			<td colspan="4" style="text-align: left;height: 26px;" class="ui-widget-content jqgrow ui-row-ltr">备注：
				${schema.REMARK }<input type="hidden" name="REMARK" value="${schema.REMARK }">
			</td>
		</tr>		
		<tr>
			<td colspan="4" style="text-align: center;height: 26px;" class="ui-widget-content jqgrow ui-row-ltr">
