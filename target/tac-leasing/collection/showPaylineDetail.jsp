...
<html>
	...
	<tr>
		...
		<!-- Marked by Michael 2012 3-27 将实际本金栏位关闭，开发净本金余额 -->
		<!--  
		<td rowspan="2" style="text-align: center;" class="ui-state-default ui-th-ltr zc_grid_head">
			实际本金
		</td>
	...
	<c:forEach items="${paylist.paylines }" var="item" varStatus="status">
		...
		<td ${bgcolorClass} style="text-align: right" class="ui-widget-content jqgrow ui-row-ltr">
			&nbsp;<fmt:formatNumber value="${item.NET_CURRENT_FINANCE }" type="currency" />
		</td>		
		<td ${bgcolorClass} style="text-align: right" class="ui-widget-content jqgrow ui-row-ltr">
			&nbsp;<fmt:formatNumber value="${item.NET_FINANCE }" type="currency" />
		</td>
		<!--		
		<td ${bgcolorClass} style="text-align: right" class="ui-widget-content jqgrow ui-row-ltr">
			&nbsp;<fmt:formatNumber value="${item.CURRENT_RENPRICE }" type="currency" />
		</td>	
		<td ${bgcolorClass} style="text-align: right" class="ui-widget-content jqgrow ui-row-ltr">
			&nbsp;<fmt:formatNumber value="${item.CURRENT_FINANCE_COST_REN }" type="currency" />
		</td>
	...
	<c:set value="${NETCURRENTFINANCE + item.NET_CURRENT_FINANCE }" var="NETCURRENTFINANCE"></c:set>
	<c:set value="${NETFINANCE + item.NET_FINANCE }" var="NETFINANCE"></c:set>
	<c:set value="${CURRENTRENPRICE + item.CURRENT_RENPRICE }" var="CURRENTRENPRICE"></c:set>
	<c:set value="${CURRENTFINANCECOSTREN + item.CURRENT_FINANCE_COST_REN }" var="CURRENTFINANCECOSTREN"></c:set>
