...
function addIrrMonthPriceRow(){
	...
	addCell(row,0,'IRRMONTHPRICEWORD', null,null);	
	addCell(row,1,'IRRMONTHPRICE',null,null);	
	addCell(row,2,'STARTANDENDWORD',null,null);	
	addCell(row,3,'STARTANDEND',value,null);
	addCell(row,4,'DELSPAN',null,null);	
	
	hideDelSpan();
}

function addCell(row, index, name,value,height){
	...
	if (name == "IRRMONTHPRICE"){
		html = html + '<input type="text"  name="PAY_IRR_MONTH_PRICE" class="input_right" style="width: 130px;" value="">';
	}
	...
	if (name == "DELSPAN"){
		html = html + '<span name="delSpan" style="display: block;"><input type="button" name="delBut" onclick="deleteRow.call(this)" value="删 除" class="ui-state-default ui-corner-all"></span>'
	}
