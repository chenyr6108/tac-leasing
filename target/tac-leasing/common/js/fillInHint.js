	...
	$.ajax({
		...
		success:function(dataBack){ 
			...
			if(parseInt(custInfo.SUM3)>0){
				...
				if((parseInt(custInfo.SUM1)+parseInt(custInfo.SUM2))>0) {
					$("#type").val("既有客户");
				} else {
					$("#type").val("新客户");
				}
			...
			if($("input[name=__action]").eq(0).val()=='creditCustomer.add') {
				$("#copy_cust_id").val(dataBack.custInfo.CUST_ID);
				if($("#type").val()=="既有客户") {
					...
					$.ajax({
						type:'post',
						url:'../servlet/defaultDispatcher',
						data:'__action=creditCustomer.getPeriodCredit&custId='+dataBack.custInfo.CUST_ID,
						dataType:'json',
						success:function(dataBack) {
