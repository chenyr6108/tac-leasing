...
<html>
	...
	<body>
		...
		<form id="form1" name="form1" method="POST" action="${ctx}/servlet/defaultDispatcher">
			<input type="hidden" name="__action" id="__action" value="productCredit.queryAllProductCreditForAuditMananger" />
...
<!-- 					<th style="text-align: center;height:26px;" class="ui-state-default ui-th-ltr zc_grid_head" style="width: 130px" colspan=4>
					...
					<input type="hidden" name="PRODUCT_ID" id="PRODUCT_ID"/>
					<input type="hidden" name="AUDIT" id="AUDIT" name="AUDIT"/>
				</tr>
				<tr>
...
<%-- 						<td style="text-align: right;"><fmt:formatNumber pattern="#,##0.00"  value="${result.LIEN_LAST_PRICE}"/>&nbsp;</td>
						...
						<td style="text-align: right;"><fmt:formatNumber pattern="#,##0.00"  value="${result.ADVANCEMACHINE_LAST_PRICE}"/>&nbsp;</td>
						...
						<td><a href="#" onclick="javaScript:location.href='${ctx }/servlet/defaultDispatcher?__action=productCredit.queryLog&PRODUCT_ID=${result.ID }'">查看</a>&nbsp;<a href="#" onclick="showProduct('${result.ID }');">添加授信</a>&nbsp;&nbsp;</td>
				</tr>		
				</c:forEach>
			</tbody>
			<tfoot>
