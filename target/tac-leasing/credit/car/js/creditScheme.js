...
function saveScheme() {
	$("#TAX_PLAN_CODE").attr("disabled",false);
	if(document.getElementById("TAX_PLAN_CODE").value!=4&&document.getElementById("contract_typew").value==7) {
		alert("直接租赁方式必须选择直租税费方案!");
		return;
	}
	if($("#TAX_PLAN_CODE").val()==5){
		if($("#outPay").val()=="" || $("#outPay").val()==0){
			alert("银行手续费为：0，请到租金测算中进行更新！");
		}
	}
	if(!checkTime()){
		return;
	...
	//zhangbo0718
		if($("input[name='APPROPRIATEFUNS']:checked").length>0){
			var appBefore=$("#APPROPIATENAME0").val();
		    var appAfter=$("#APPROPIATENAME1").val();
				if($("input[name='APPROPRIATEFUNS']:checked").length==1){
					if((appBefore=="" || appBefore==null) && (appAfter=="" || appAfter==null)){
	            		alert("请填写对应拨款方式中：'拨款给'栏位");
	            		return false;
	            	}
			}
			if($("input[name='APPROPRIATEFUNS']:checked").length==2){
				if(appBefore=="" || appBefore==null || appAfter=="" || appAfter==null){
            		alert("请填写对应拨款方式中：'拨款给'栏位");
            		return false;
            	}
...
/*    if(!checkInteger(PLEDGE_LAST_PERIOD.value)){
	...
	*/  
    ...
    if(PAY_WAY.value == 13 ||PAY_WAY.value == 11){
    	if(parseFloat(HEAD_HIRE.value)<0){
...
function commitCredit(id, cust_id) { 
	...
	$.ajax({
		...
		data:'__action=creditReport.checkBankCharge&creditId='+id,
		dataType:'json',
		async:false,
	...
	$.ajax({
		type:'post',
		url:'../servlet/defaultDispatcher',
		data:'__action=creditReport.getunUp&credit_id='+id + '&cust_id='+cust_id,
		dataType:'json',
		success: function(dataBack){ 
		if(dataBack.unUpList.length==0){
			...
			$.ajax({
				type:'post',
				url:'../servlet/defaultDispatcher',
				data:'__action=creditReport.getIsCanSubmit&credit_id='+id+'&fromWhere=credit',
				dataType:'json',
				success: function(dataBack){ 
					if(dataBack.content.toString() == ""){
						...
						$.ajax({
							...
							success:function(data) { 
								$("#__action").val("creditReport.selectCreditForCommit");
								$("#form1").submit();
							}
						});
					}else{
			...
			error: function() {
				...
				for(var i=0;i<dataBack.unUpList.length;i++){ 
...
function showImportEqipItem(contratcType){
	if(contratcType==7 || contratcType==9){
		$("#isImportEqipDiv").show();
		$("#isImportEqip").attr("checked",false);
	}else{
		$("#isImportEqipDiv").hide();
		$("#isImportEqip").attr("checked",false);		
	}
	$("#PAY_WAY").find("option").each(function(){
		$(this).remove();
	});
	if(contratcType ==13){
		for(var i=0;i<payWays.length;i++){
			if(payWays[i].value==13 || payWays[i].value==11){
				$("#PAY_WAY").append("<option value=\""+payWays[i].value+"\">"+payWays[i].name+"</option>")
			}			
		}
	}else{
		for(var i=0;i<payWays.length;i++){
...
function changePayWayForCar(contratcType){
	if(contratcType==10 || contratcType==12 || contratcType==13){
		$("#OnlinetBank").attr("checked",true)
		$("#cheque").attr("checked",false)
		$("#chequeDiv").hide();
	}else{
		$("#OnlinetBank").attr("checked",false)
