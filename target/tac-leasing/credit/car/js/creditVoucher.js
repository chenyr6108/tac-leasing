...
function addVoucherCorp(){
	...
	$("#addVoucherCorp_sec").load("../servlet/defaultDispatcher?__action=creditVoucher.gotoAddVoucherCorp&credit_id="+credit_id+"&showFlag="+showFlag);
}
// 弹出层(自然人)
...
function delCorp(msg,id){
	$("#"+msg).remove();
	var url = "../servlet/defaultDispatcher?__action=creditVoucher.deleteCorp&pjccc_id="+id;
	$.getJSON(url);
}
...
//	}
	...
	if(!confirm("确定更新担保人'"+document.getElementById('CORP_NAME_CN'+msg).value+"'信息?")) {
		return;
	}
	//$("#__action"+msg).val("creditVoucher.upCreditCorp");
...
//	}
	...
	*/
	if(!confirm("确定更新担保人'"+document.getElementById('natu_name'+msg).value+"'信息?")) {
		return;
	}
