...
function loadEquipmentManufacturer(id,value){
	$.ajax({
		...
		data:{__action:'suplEquipment.getAllTypesByManufacturer',
			MANUFACTURER:value},
		dataType:'json',
		success:function(dataBack){   
...
//	 	}
	 ...
	 if(type=='select'&&name=='SUPPIER'){
	 	html=html+"<select id='"+name+sed+"' name='"+name+"' class='"+cls+"' onchange='showApplyGrants(SUPPIER"+sed+",this.value);$(\"#SUPPIER_NAME"+sed+"\").val(this.options[this.selectedIndex].text);"+"isBuyBackFunction(\"SUPPIER"+sed+"\")"+"'>"
	 	+"<option value='-1'>--供应商--</option></select>";	
	 }
	 if(type=='select'&&name=='UNIT'){
	 	html=html+"<select id='"+name+sed+"' name='"+name+"' class='"+cls+"' >"
	 	+"<option value='台'>-台-</option><option value='辆'>-辆-</option><option value='套'>-套-</option><option value='件'>-件-</option><option value='条'>-条-</option><option value='批'>-批-</option></select>";	
	 }
	 ...
	 if(type=='a'){
	 	html=html+"<a href='javascript:void(0)' onclick='addOption();deleteRow.call(this)'><span id='del"+sed+"'><img src='../images/u103.gif'>删除</span></a>"
