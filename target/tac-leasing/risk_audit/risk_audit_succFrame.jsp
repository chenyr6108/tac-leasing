					...
					$.ajax({
						...
						success: function(data) {
							...
							if(data.flag) {
								...
								$("input[name=memo]").eq(1).attr("disabled","disabled");
							}
						}
					});
				//}
									...
									<%-- <div id="tabs">
										<ul>
											...
											<li style="width: 100px;text-align: center;">
												<a href="#tabs-1" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=riskAudit.selectRiskAuditForSucc_zulin&credit_id=${credit_id}&showFlag=0'">现场调查报告</a>
											</li>
											</c:if>
											<c:choose>
											<c:when test="${typeOfContract eq '3'}">
												<li style="width: 100px;text-align: center;" >
												<a href="#tabs-2"
								        			onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=riskAudit.selectRiskAuditForSucc_fen&credit_id=${credit_id}&showFlag=1&typeOfContract=${typeOfContract}'">信用评分表</a>
												</li>
											</c:when>
											<c:otherwise>
											    <li style="width: 100px;text-align: center;"><a href="#tabs-2" onclick="javascript:location.href='${ctx }/servlet/defaultDispatcher?__action=riskAudit.selectRiskAuditForSucc_fen&credit_id=${credit_id}&showFlag=${showFlag }&prc_id=${prc_id }'">测评记分表</a></li>
										    </c:otherwise>
										    </c:choose>
										</ul>
										<c:if test="${flag == 1 }">
										<div id="tabs-1">	 --%>
												<jsp:include page="risk_audit_zulin.jsp"></jsp:include> 
												<%-- <br/>
										</div>
										</c:if> --%>
										<%-- <div id="tabs-2">	
										        <c:choose>
							                    <c:when test="${typeOfContract eq '3'}">	
							                        <jsp:include page="risk_autit_zhongChen.jsp"></jsp:include>
							                    </c:when>
							                    <c:otherwise>
												    <jsp:include page="risk_autit_fen.jsp"></jsp:include>
												</c:otherwise>
												</c:choose>
												<br/>
										</div> --%>
