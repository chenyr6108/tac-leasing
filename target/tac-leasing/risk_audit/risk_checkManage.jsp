...
<tr>
	<td style="text-align: left;height: 24px;" class="ui-state-default ui-th-ltr zc_grid_head" colspan="3">
		<b style="font-size: 14px;">支票还款<input type="checkbox" name="IS_CHECK_PAY" id="IS_CHECK_PAY" <c:if test="${creditshemadetail.IS_CHECK_PAY==1 }">checked="checked"</c:if>/><input type="hidden" name="IS_CHECK_PAY_VALUE" id="IS_CHECK_PAY_VALUE" value="${creditshemadetail.IS_CHECK_PAY }"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;增加支票还款期数</b>&nbsp;<img src="${ctx }/images/u100.gif"><a href="javascript:void(0)" style="cursor: pointer;" onclick="addCheckPaylines();">添加</a>
	</td>
...
<c:forEach var="item" items="${checkPaylines}" varStatus="status">
	<tr>
		...
		<td   style="text-align:  left;font-size:14px;" class="ui-widget-content jqgrow ui-row-ltr">
			第<select name="CHECK_PAY_START" onchange="changeIrrMonthPriceStrat(this);">
				...
				</c:forEach></select>期到第<select name="CHECK_PAY_END" onchange="changeIrrMonthPriceEndTab1(this);">
				<c:forEach begin="1" end="${creditshemadetail.LEASE_TERM}" var="item1" step="1">
					<option value="${item1 }" <c:if test="${item.CHECK_END eq item1}">selected="selected"</c:if>>${item1 }</option>
				</c:forEach></select>期
		</td>
		<td   style="text-align:  left;font-size:14px;" class="ui-widget-content jqgrow ui-row-ltr">
			<span name="delSpanTab1"<c:if test="${status.count ne fn:length(checkPaylines)}">style="display: none;"</c:if>><input type="button" name="delBut" onclick="deleteIrrMonthPriceRowTab1.call(this)" value="删 除" class="ui-state-default ui-corner-all"></span>
		</td>
	</tr>
</c:forEach>
</table>