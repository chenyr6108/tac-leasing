...
<html>
	<head>
		<script type="text/javascript">
			$(function() {
				...
				$("#queryDate").datepicker($.datepicker.regional['zh-CN']);
				var d1=new Date($("#queryDate").val().replace(/-/g,"/"));
				var d2=new Date();
				var d3=new Date(d2.getFullYear().toString()+"/"+(d2.getMonth()+1).toString()+"/"+d2.getDate().toString());
				if(d1>=d3) {
					$("input[name=exportBtn]").attr("disabled","disabled");
				} else {
					$("input[name=exportBtn]").attr("disabled",false);
				}
			});
			//选择tab
			...
			function exportDailyReport() {
				window.parent.callBirt("birtReport/dailyReport/dailyReport.rptdesign","xls",{"queryDate":$("#queryDate").val(),"companyCode":$("#companyCode").val()});
			}
			function exportCashReport() {
				window.parent.callBirt("birtReport/dailyReport/cashReport.rptdesign","xls",{"queryDate":$("#queryDate").val(),"companyCode":$("#companyCode").val()});
			}
			function exportFundReport() {
				window.parent.callBirt("birtReport/dailyReport/fundReport.rptdesign","xls",{"queryDate":$("#queryDate").val(),"companyCode":$("#companyCode").val()});
			}
			function exportBalanceReport() {
				window.parent.callBirt("birtReport/dailyReport/balanceReport.rptdesign","xls",{"queryDate":$("#queryDate").val(),"companyCode":$("#companyCode").val()});
			}
		</script>
	...
	<body>
		<div id="tabs">
		<ul>
			<li style="width: 100px;text-align: center;"><a href="#tabs-1" onclick="selectCard('queryCashDailyReport',0)">现金销帐</a></li>
			<li style="width: 100px;text-align: center;"><a href="#tabs-2" onclick="selectCard('queryFundDailyReport',1)">暂收款销帐</a></li>
			<li style="width: 120px;text-align: center;"><a href="#tabs-3" onclick="selectCard('queryBalanceDailyReport',2)">暂收款余额变动表</a></li>
		</ul>
				        ...
				        <td width="600" class="ss_bigt" valign="center" >
				         <table>
				         	<tr>
				         		...
				         		<td>
				         			<input name="queryDate" id="queryDate" style="width: 80px;" readonly="readonly" value="${queryDate }" onchange="controlBtn()">
				         		</td>
				         		<td>查询内容：</td>
		    	...
		    	<table cellspacing="0" cellpadding="0" border="0" class="panel_table">
					<tr>
						<td style="border-right: 0px;">
							<input class="panel_button" type="button" value="导出销帐日报表" onclick="exportDailyReport()" name="exportBtn">&nbsp;
