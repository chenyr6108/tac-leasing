...
<script type="text/javascript">
	...
	function inputReport(visit_id){
		...
		//$("#assignForm").submit();
		var url = "${ctx }";
		window.parent.openNewTab(url,"visitCommand.inputReport&visit_id=" + visit_id + "&show_type=U", "访厂报告");
	...
	function showReport(visit_id){
		...
		//$("#assignForm").submit();
		var url = "${ctx }";
		window.parent.openNewTab(url,"visitCommand.inputReport&visit_id=" + visit_id + "&show_type=F", "访厂报告");
	}
...
<input type="hidden" name="__action" value="visitCommand.visitReportManager"/>
<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
 		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;" >&nbsp;&nbsp;<c:if test="${isSalesDesk eq 'Y' }">访厂客户清单</c:if><c:if test="${isSalesDesk ne 'Y' }">访厂管理</c:if></span>
</div>
...
<table class="grid_table">
	...
	<tbody>
		...
		<tr>
			...
			<td><a href="javascript:void(0);" onclick="getCredit('${item.ID}', '');">[查看]</a>&nbsp;&nbsp;<%-- <a href="javascript:void(0)" onClick="creditfiles('${item.ID }','${item.CONTRACT_TYPE+1 }');">[资料]</a> --%></td>
			<td id="test_td">
				<c:if test="${assignRole }"><c:if test="${item.VISIT_STATUS == 1 }"><a id="noneVisit" href="javascript:void(0);" onclick="noneVisit('${item.VISIT_ID}', '<fmt:formatDate value="${item.MODIFY_DATE}" pattern="yyyy-MM-dd HH:mm:ss SSS"/>', '${item.ID}')">[无需访厂]</a>&nbsp;&nbsp;<a id="assignVisit" href="javascript:void(0);" onclick="assignVisit('${item.VISIT_ID}', '<fmt:formatDate value="${item.MODIFY_DATE}" pattern="yyyy-MM-dd HH:mm:ss SSS"/>', '${item.ID}')">[分配]</a>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="rejectVisit('${item.VISIT_ID}', '<fmt:formatDate value="${item.MODIFY_DATE}" pattern="yyyy-MM-dd HH:mm:ss SSS"/>', '${item.ID}')">[驳回]</a>&nbsp;&nbsp;</c:if></c:if><c:if test="${item.VISIT_STATUS == 2 }"><c:if test="${assignRole }"><a id="assignVisit" href="javascript:void(0);" onclick="changeAssign('${item.VISIT_ID}')">[修改分配]</a>&nbsp;&nbsp;</c:if><c:if test="${editReportRole }"><a href="javascript:void(0);" onclick="inputReport('${item.VISIT_ID}')">[填写访厂报告]</a>&nbsp;&nbsp;</c:if></c:if><c:if test="${item.VISIT_STATUS == 3 }"><c:if test="${editReportRole }"><c:if test="${empty item.REPORT_STATUS or item.REPORT_STATUS == 0 }"><a href="javascript:void(0);" onclick="inputReport('${item.VISIT_ID}')">[修改访厂报告]</a></c:if></c:if><a href="javascript:void(0);" onclick="showReport('${item.VISIT_ID}')">[查看访厂报告]</a></c:if>&nbsp;&nbsp;<c:if test="${item.VISIT_STATUS == 4 }"><a href="javascript:void(0);" onclick="showNoneVisitMemo('${item.VISIT_ID}')">[查看理由]</a></c:if></td>
				...
				<table cellspacing="1" cellpadding="3" border="0" class="ui-jqgrid-htable zc_grid_title">
					<tr class="ui-state-default ui-jqgrid-hdiv">
						...
						<td><select id="none_visit_reason" name="none_visit_reason">
							...
							<option value="共案免访厂">共案免访厂</option>
