...
<body>
	<div class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix" style="height: 30px">
		<span class="ui-jqgrid-title" style="line-height: 30px; font-size: 15px;">&nbsp;修改资讯需求单</span>
	</div>
	<br>
	...
	<form action="${ctx}/servlet/defaultDispatcher" name="form1" id="form1" method="post" enctype="multipart/form-data">
		<input type="hidden" id="__action" name="__action" value="modifyOrderCommand.editDemand">
		<input type ="hidden" value="${pageContext.session.id}" id="sessionId"/>
		<input type="hidden" value="" name="upFiles" id="files"/>
		<input type="hidden" value="${demand.ID }" name="demandId" id="demandId"/>
		<input type="hidden" value="${demand.HOURS }" name="HOURS"/>
		<input type="hidden" value="1" id="fileValid"/>
		<c:if test="${demand.DEMAND_TYPE eq 1  }">
			<input type="hidden" value="${demand.COUNTERSIGN_CODE_ORDER }" id="chooseCodes" name="COUNTERSIGN_CODE_ORDER" />
			<input type="hidden" value="业管部 → 资讯部 → 结束" id="chooseCodesName" name="chooseCodesName" />
			<input type="hidden" value="${demand.SENIOR_SIGN }" id="seniorSignSub" name="seniorSign" />
		</c:if>
		<br>
										...
										<table cellspacing="0" cellpadding="0" border="0" class="panel_table" id="modifyOrderTable">
											<tr id="orderTr">
												...
												<td style="height: 30px;">
													<input type="text" name="HOPE_COMPLETE_DATE" value="${demand.HOPE_COMPLETE_DATE}" style="width: 80px; height: 18px;" readonly="readonly" id="hopeDate">
												</td>
											</tr>
											<tr>
												<th style="height: 30px; width:15%">摘要：</th>
												<td colspan="5" align="left" style="height: 30px;">
													<input style="width:40%;height:28px;" maxlength="40" type="text" id="SUMMARY" name="SUMMARY" value="${demand.SUMMARY }" />
											...
											<tr>
												...
												<td colspan="5" style="height: 100px;">
													<textarea rows="14" cols="100" style="width:100%" id="content" name="CONTENT">${demand.CONTENT }</textarea>
												</td>
											</tr>
											<c:if test="${demand.DEMAND_TYPE eq 1 }">
												<tr>
													...
													<td colspan="5" style="padding:4px 0 0 4px; height:30px;">
														<a id="alter" href="javascript:void(0);" title="点击修改" onclick="countersignList('','',5);">
															<c:forEach items="${comCounList}" var="d" varStatus="index">${d.FLAG }&nbsp;→&nbsp;</c:forEach>结束
														</a>
													</td> 
												</tr>
											</c:if>
											<tr>
												<th colspan="2" style="height: 30px;text-align: right;">
													资讯单附件：<input type="hidden" name="fileDelIds" id="fileDelIds" value="" />
												</th>
												<td colspan="4">
											...
											<tr>
												<th colspan="2" style="height: 30px; padding:2px; text-align: right;">
													添加附件(单个文件大小限2M以下)：
												</th>
											...
											<tr>
												<td colspan="6" style="height:40px; text-align: center;">
													<input class="ui-state-default" id="sub" type="button" value="修改申请" style="height: 30px;width: 90px" onclick="updateDemand();"/>
												</td>
											</tr>
										</table>
									</td>
						...
						<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title">
							...
							<tr class="ui-widget-content jqgrow ui-row-ltr">
								...
								<td style="text-align: center; height:36px; color:#2E6E9E; font-weight:bold;">
									<input style="vertical-align: middle;" <c:if test="${demand.SENIOR_SIGN eq 2}">disabled=disabled;</c:if> id="seniorSign" name="seniorSign" <c:if test="${demand.SENIOR_SIGN eq 1 || demand.SENIOR_SIGN eq 2}">checked=checked</c:if> type="checkbox" value="Y" />&nbsp;添加总经理高阶签核
								</td>
							</tr>
							<tr class="ui-widget-content jqgrow ui-row-ltr">
