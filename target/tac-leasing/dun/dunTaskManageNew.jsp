	          	...
	          	<table width="97%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
				      ...
				      <td width="8">&nbsp;</td>
				        ...
				        <td style="min-width:756px" class="ss_bigt" >
				          ...
				          <td>原始办事处：</td> 	
				          <td><select id="ORG_COMPANY" name="ORG_COMPANY" >
				         	 <option value="">全部</option>
				          <c:forEach items="${companyList }" var="item" >
				          	<option value="${item.DECP_ID}" <c:if test="${item.DECP_ID == ORG_COMPANY}">selected='selected'</c:if>>${item.DECP_NAME_CN }</option>
				          </c:forEach>
				          </select>
				          </td>
				          <td>核准人：</td>
				          <td><input type="text"  name="approvalUserName" id="approvalUserName" style="width:80px; height:18px;" value="${approvalUserName }"></td>
				          </tr>
				          <tr>
					          <td>原始业务员：</td>
				...
				<table cellspacing="0" cellpadding="0" border="0" class="ui-jqgrid-htable zc_grid_title" id="#table1">
					...
					<table class="grid_table">
						...
						<c:forEach items="${pagingInfo.resultList}" var="item" varStatus="status">
							<tr>
								<td style="text-align: center;" onclick="$dunTask.showDunList('${item.CUST_CODE }','${item.CUST_ID}','${dun_date}','${item.LEASE_CODE}')">
									<c:choose>
