...
<body>
	<form action="../servlet/defaultDispatcher" name="form1" id="form1"
		...
		<input type="hidden" name="__action" id="__action"
			value="unnaturalCaseCommand.queryPendingApproveCase">
		<div
			class="ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix"
			style="height:30px;">
			<span class="ui-jqgrid-title"
				style="line-height:30px;font-size:15px;">&nbsp;&nbsp;提交审查逾12天未核准(截至<c:forEach var="item" items="${dateList}"><c:choose><c:when test="${DATE == null }"><c:if test="${item.FLAG eq 'Y' }">${item.DATE }</c:if></c:when><c:otherwise><c:if test="${item.DATE eq DATE }">${item.DATE }</c:if></c:otherwise></c:choose></c:forEach>&nbsp;17:30:00)</span>
		</div>
		<div class="zc_grid ui-jqgrid ui-widget ui-widget-content ui-corner-all" id="main">
			...
			<page:pagingToolbarTop pagingInfo="${pagingInfo }"/>
				...
				<tr class="ui-jqgrid-labels">
					...
					<th class="ui-state-default ui-th-ltr zc_grid_head">
						<page:pagingSort orderBy="minCommitDate" pagingInfo="${pagingInfo }"><strong>初次提交风控时间</strong></page:pagingSort>
					</th>
					<th class="ui-state-default ui-th-ltr zc_grid_head">
