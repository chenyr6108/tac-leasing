		...
		$.ajax({
				...
				success: function(json){
					...
					if (json.password == 1) {
						...
						$("#lastResult").append("<font class='cc'>修改成功</font>");
...
function newPasswordValue(){
	...
	$("#behindpassword1").empty();
		...
		if (!(val1.length == 0)) {
			...
			if (!regex.test(val1)) {
				$("#behindpassword1").append("<font color='red'>密码格式不正确！！</>")
			...
			if ($("#oldPassword2").val() == $("#password1").val()) {
				$("#behindpassword1").empty();
				$("#behindpassword1").append("<font color='red'>旧密码与新密码相同!!</>");
